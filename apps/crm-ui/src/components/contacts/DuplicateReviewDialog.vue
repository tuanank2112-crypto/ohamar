  ...
  <v-dialog v-model="show" :max-width="1100" :fullscreen="$vuetify.display.smAndDown" scrollable>
    <v-card class="dup-dialog">
      ...
      <div class="dup-filters" v-if="duplicateTotal > 0">
        ...
        >
          <CoolIcon :name="opt.icon" :size="15" class="chip-icon" />
          {{ opt.label }}
        ...
        <div v-else class="dup-groups">
          ...
          >
            <header class="group-head">
              ...
              <span v-if="isConflict(group.matchType)" class="conflict-warn" :title="conflictTooltip(group.matchType)"> <CoolIcon name="Warning" :size="14" /> Cần xem kỹ </span>
...
async function onMerge(groupId: string) {
  ...
  if (!(await confirm({
    title: `Gộp ${secondaries.length} KH vào “${primary?.fullName || 'KH gốc'}”?`,
    message: 'Toàn bộ nick chăm, hội thoại và lịch hẹn sẽ chuyển sang KH gốc. KH trùng sẽ được ẩn nhưng dữ liệu vẫn được giữ.',
    tone: 'danger',
    confirmText: 'Gộp khách hàng',
  }))) return;
...
async function onDismiss(groupId: string) {
  if (!(await confirm({
    title: 'Bỏ qua nhóm trùng này?',
    message: 'Nhóm sẽ được đánh dấu là không trùng và không còn xuất hiện trong danh sách xử lý.',
    confirmText: 'Bỏ qua nhóm',
  }))) return;
...
.filter-chip.active .chip-count { background: var(--mc-surface); color: var(--smax-primary); }
...
.badge-strong { background: rgba(76,175,80,0.15); color: var(--mc-success); }
.badge-warn   { background: rgba(255,152,0,0.18); color: var(--mc-warning); }
.badge-info   { background: rgba(33,150,243,0.15); color: #aeb7ff; }
.badge-soft   { background: var(--smax-grey-100); color: var(--smax-grey-700); }
.group-count { font-weight: 600; color: var(--smax-grey-700); }
.conflict-warn { color: var(--mc-warning); font-weight: 600; font-size: 11.5px; cursor: help; }
.group-spacer { flex: 1; }
.group-similarity {
...
.col-role-badge.primary {
  background: rgba(76,175,80,0.15); color: var(--mc-success);
}
.col-role-badge.duplicate {
  background: rgba(255,152,0,0.18); color: var(--mc-warning);
}
...
.richness-pill.has { background: rgba(33,150,243,0.10); color: #aeb7ff; font-weight: 500; }
...
.field.highlight .field-value { font-weight: 600; color: var(--mc-warning); }
...
.role-badge.primary { background: rgba(76,175,80,0.15); color: var(--mc-success); }
.role-badge.secondary { background: rgba(255,152,0,0.18); color: var(--mc-warning); }
