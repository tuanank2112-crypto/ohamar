  ...
  <teleport to="body">
    <div v-if="modelValue" class="cpd-overlay" @click.self="close">
      <div class="cpd-modal" role="dialog" aria-modal="true">
        <div v-if="loading" class="cpd-loading"><CoolIcon name="Timer" :size="14" /> Đang tải hồ sơ khách hàng…</div>
        <div v-else-if="error" class="cpd-error"><CoolIcon name="Warning" :size="14" /> {{ error }}</div>
        ...
        <template v-else-if="c || isCreate">
          ...
          <header v-if="isCreate" class="cpd-head cpd-head-create">
            ...
            <button class="cpd-x" @click="close" title="Đóng"><CoolIcon name="Close_MD" :size="14" /></button>
          </header>
          ...
          <header v-else-if="c" class="cpd-head">
            ...
            <div class="cpd-head-main">
              ...
              <div class="cpd-sub">
                <span v-if="primaryPhone"><CoolIcon name="Mobile" :size="14" /> {{ formatVnPhone(primaryPhone) }}</span>
                <span v-if="c.email"><CoolIcon name="Mail" :size="14" /> {{ c.email }}</span>
                <span v-if="locationLine"><CoolIcon name="Map_Pin" :size="14" /> {{ locationLine }}</span>
                <span v-if="c.createdAt"><CoolIcon name="Calendar" :size="14" /> KH từ {{ formatDate(c.createdAt) }}</span>
              </div>
              <div class="cpd-pills">
                ...
                <span v-for="t in (c.tags || []).slice(0, 5)" :key="t" class="crmtag"><CoolIcon name="Tag" :size="14" /> {{ t }}</span>
              </div>
            </div>
          ...
          <div class="cpd-body">
            ...
            <section v-if="activeTab === 'overview'" class="cpd-pane">
              <div class="cpd-grid2">
                ...
                <div class="cpd-card">
                  <h4><CoolIcon name="User_01" :size="14" /> Thông tin cá nhân</h4>
                  <div class="kv">
                    ...
                    <span class="v">
                      <div class="phones-edit">
                        ...
                        <div v-for="(p, i) in form.extraPhones" :key="i" class="phone-row">
                          ...
                          <span class="phone-rm" @click="form.extraPhones.splice(i, 1)" title="Xoá số"><CoolIcon name="Close_MD" :size="14" /></span>
                        </div>
                ...
                <!-- Chăm sóc & phân loại -->
                <div class="cpd-card">
                  <h4><CoolIcon name="Compass" :size="14" /> Chăm sóc & phân loại</h4>
                  <div class="kv">
                    ...
                    <span class="v">
                      ...
                      <div v-else class="tag-edit">
                        ...
                        <span v-for="(t, i) in form.tags" :key="i" class="crmtag editable">
                          {{ t }}<span class="rm" @click="form.tags.splice(i, 1)"><CoolIcon name="Close_MD" :size="14" /></span>
                        </span>
                        <input
              ...
              <div v-if="!isCreate" class="cpd-attr">
                ...
                <div v-if="hasScoreBd" class="attr-card full">
                  <h4 class="attr-h"><span class="ic"><CoolIcon name="Chart_Bar_Vertical_01" :size="14" /></span> Lead Score — {{ Math.round((c?.displayLeadScore ?? c?.leadScore) || 0) }}/100 (4 chiều)</h4>
                  <div class="bd-row">
                ...
                <div class="attr-tablewrap">
                  ...
                  <table class="attr-tb">
                    ...
                    <tbody>
                      ...
                      <tr class="attr-grp"><td colspan="3"><CoolIcon name="Settings" :size="14" /> Hệ thống & nguồn (ngoài biến)</td></tr>
                      <tr class="attr-itrow">
                  ...
                  <div class="attr-foot"><CoolIcon name="Bulb" :size="14" /> 36 biến dùng chung cho ô chat · mẫu tin · Khối · Sequence. Cột giá trị lấy thật từ hồ sơ khách + nick đang chọn (khớp lúc render tin). Nhóm "ngoài biến" là thông tin hệ thống, không chèn vào tin.</div>
                </div>
              </div>

              <div v-if="!isCreate" class="cpd-aggnote"> <CoolIcon name="Bulb" :size="14" /> Field gắn nhãn <span class="agg">tổng hợp</span> tự cập nhật từ các nick chăm (tab "Nick chăm").
              ...
              <template v-else>
                ...
                >
                  ...
                  <div class="s-r s-r2">
                    ...
                    <span class="s-meta">
                      <span><CoolIcon name="Archive" :size="14" /> <b>{{ f.totalInbound ?? 0 }}</b></span>
                      <span><CoolIcon name="Paper_Plane" :size="14" /> <b>{{ f.totalOutbound ?? 0 }}</b></span>
                    </span>
                  </div>
            ...
            <section v-else-if="activeTab === 'timeline'" class="cpd-pane">
              ...
              <template v-else>
                <div v-for="t in timeline" :key="t.id" class="tl-item">
                  <span class="dot"></span>
                  <div class="tx"><CoolIcon :name="t.icon" :size="14" /> {{ t.text }}<div class="tt">{{ formatRelative(t.at) }}</div></div>
                </div>
          ...
          <footer class="cpd-foot">
            <template v-if="isCreate">
              ...
              <button class="btn" @click="close"><CoolIcon name="Close_MD" :size="14" /> Hủy</button>
            </template>
            <template v-else-if="c">
              <button v-if="c.hasZalo" class="btn primary" @click="goChat"><CoolIcon name="Chat" :size="14" /> Mở chat Zalo</button>
              <button v-else class="btn virtual" @click="goChat"><CoolIcon name="Lock" :size="14" /> Mở chat nội bộ</button>
              <!-- ⚡ Marketing (automation) là tính năng EE — ẩn ở Community -->
              <button v-if="isExtension" class="btn" @click="$emit('automation', c)">⚡ Marketing</button>
              <span class="spacer"></span>
              <button class="btn" :disabled="saving" @click="save">{{ saving ? '⏳ Đang lưu…' : '💾 Lưu thay đổi' }}</button>
              <button class="btn" @click="close"><CoolIcon name="Close_MD" :size="14" /> Đóng</button>
            </template>
          </footer>
        </template>
      </div>
...
.cpd-modal {
  ...
  max-height: calc(100vh - 64px); background: var(--mc-surface); border-radius: 12px;
...
.gtag.male { background: rgba(30, 136, 229, 0.13); color: #aeb7ff; }
...
.cpd-scorebig .n.sc-hi { color: var(--mc-success); } .cpd-scorebig .n.sc-mid { color: var(--mc-warning); } .cpd-scorebig .n.sc-lo { color: var(--mc-danger); }
...
.z-yes { background: rgba(0, 200, 83, 0.14); color: var(--mc-success); }
.z-no { background: rgba(255, 61, 0, 0.12); color: var(--mc-danger); }
.z-unk { background: var(--smax-grey-100); color: var(--mc-text); }
...
.crmtag { font-size: 11px; border: 1px solid var(--smax-grey-300); border-radius: 9px; padding: 1px 8px; color: var(--smax-grey-700); background: var(--mc-surface); }
...
.cpd-tab.active { color: var(--smax-primary); border-bottom-color: var(--smax-primary); font-weight: 700; background: var(--mc-surface); }
...
.agg { font-size: 9px; background: rgba(41, 98, 255, 0.1); color: #aeb7ff; border-radius: 5px; padding: 1px 5px; font-weight: 600; }
...
.cpd-in:hover { border-color: var(--smax-grey-300); background: var(--mc-surface); }
.cpd-in:focus { outline: none; border-color: var(--smax-primary); background: var(--mc-surface); text-align: left; }
...
.cpd-aggnote { margin-top: 14px; font-size: 11.5px; color: var(--smax-grey-700); background: var(--mc-surface-alt); border: 1px solid var(--smax-grey-200); border-radius: 7px; padding: 9px 13px; }
...
.attr-card { border: 1px solid var(--line, var(--mc-line)); border-radius: var(--r-md, 10px); overflow: hidden; background: var(--surface, #fff); }
.attr-card.full { grid-column: 1 / -1; }
.attr-h { margin: 0; padding: 9px 13px; font-size: 12px; font-weight: 700; color: var(--brand-700, #0b5880); background: var(--surface-2, #f7f9fc); border-bottom: 1px solid var(--line, var(--mc-line)); display: flex; align-items: center; gap: 7px; }
...
.bd-t { height: 6px; border-radius: 3px; background: var(--line, var(--mc-line)); overflow: hidden; }
.bd-t i { display: block; height: 100%; border-radius: 3px; transition: width .3s; }

/* ════ P4: Bảng biến cá nhân hóa (Nhãn | {code} | giá trị thật) ════ */
.attr-tablewrap { border: 1px solid var(--line, var(--mc-line)); border-radius: var(--r-md, 10px); overflow: hidden; background: var(--surface, #fff); }
.attr-tbhead { display: flex; align-items: center; gap: 10px; background: var(--surface-2, #f7f9fc); border-bottom: 1px solid var(--line, var(--mc-line)); padding-right: 12px; }
...
.cpd-in.sm { height: 28px; padding: 0 8px; font-size: 12px; border: 1px solid var(--line, var(--mc-line)); border-radius: 6px; background: var(--surface, #fff); color: var(--ink, #141a24); max-width: 200px; }
...
.attr-tb thead th { text-align: left; font-size: 10.5px; text-transform: uppercase; letter-spacing: .03em; color: var(--ink-4, #97a0b3); font-weight: 700; padding: 7px 13px; border-bottom: 1px solid var(--line, var(--mc-line)); background: var(--surface, #fff); }
...
.attr-foot { padding: 8px 13px; font-size: 11px; color: var(--ink-4, #97a0b3); background: var(--surface-2, #f7f9fc); border-top: 1px solid var(--line, var(--mc-line)); }
...
.strip { background: var(--mc-surface); border: 1px solid var(--smax-grey-200); border-left: 5px solid var(--smax-grey-300); border-radius: 6px; padding: 9px 12px; margin-bottom: 7px; }
...
.strip.kb-off { border-left-color: var(--mc-text); opacity: 0.72; }
...
.winb { font-size: 10px; background: rgba(0, 200, 83, 0.13); color: var(--mc-success); border-radius: 6px; padding: 1px 6px; }
.kb { font-size: 11px; font-weight: 600; border-radius: 9px; padding: 1px 8px; }
.kbY { background: rgba(0, 200, 83, 0.15); color: var(--mc-success); } .kbP { background: rgba(255, 145, 0, 0.16); color: var(--mc-warning); }
.kbI { background: rgba(33, 150, 243, 0.14); color: #aeb7ff; } .kbO { background: var(--smax-grey-100); color: var(--mc-text); }
.chatdot { font-size: 11px; color: var(--mc-success); } .chatdot.off { color: var(--mc-text); }
...
.sc-hi { background: rgba(0, 200, 83, 0.15); color: var(--mc-success); } .sc-mid { background: rgba(255, 145, 0, 0.15); color: var(--mc-warning); } .sc-lo { background: rgba(255, 61, 0, 0.13); color: var(--mc-danger); }
.s-r2 { font-size: 11.5px; }
.s-sale { color: var(--smax-grey-700); } .s-sale b { color: var(--smax-text); }
.ftag { font-size: 10.5px; border: 1px solid #42a5f5; color: #aeb7ff; border-radius: 9px; padding: 1px 7px; font-weight: 600; }
...
.who.kh { background: rgba(0, 200, 83, 0.12); color: var(--mc-success); } .who.sale { background: rgba(33, 150, 243, 0.1); color: #aeb7ff; }
...
.btn { border: 1px solid var(--smax-grey-300); background: var(--mc-surface); border-radius: 7px; padding: 8px 16px; font-size: 13px; cursor: pointer; color: var(--smax-grey-700); font-weight: 500; font-family: inherit; }
...
.btn.virtual { background: rgba(251,191,36,.14); color: var(--mc-warning); border-color: #ffcc80; }
