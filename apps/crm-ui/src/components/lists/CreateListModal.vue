...
<template>
  <div v-if="modelValue" class="clm-overlay" @click.self="$emit('update:modelValue', false)">
    <div class="clm-modal">
      <div class="clm-head">
        <h3 class="clm-title"><CoolIcon name="File_Add" :size="18" /> Tạo tệp khách hàng mới</h3>
        <button class="clm-x" aria-label="Đóng" @click="$emit('update:modelValue', false)"><CoolIcon name="Close_MD" :size="16" /></button>
      </div>
      <div class="clm-body">
        <!-- Tên + Icon -->
        <div class="clm-field">
        ...
        <div class="clm-field">
          ...
          <div class="clm-icon-picker">
            <button
              ...
              @click="iconEmoji = ic"
            ><CoolIcon :name="ic" :size="18" /></button>
          </div>
        </div>
        ...
        <div class="clm-tabnav">
          ...
          >
            <CoolIcon :name="t.icon" :size="15" class="clm-tab-ico" /> {{ t.label }}
          </button>
        </div>
        ...
        <div v-if="activeTab === 'paste'">
          <div class="clm-field">
            <label class="clm-label">
              ...
              <span class="clm-pill" title="Định dạng: SĐT trước, tên sau. KHÔNG có ghi chú riêng — nếu cần lời mời/tin nhắn riêng cho từng KH, dùng tab CSV/Excel."> <CoolIcon name="Info" :size="14" /> SĐT trước, tên sau </span>
            </label>
            <textarea
              class="clm-textarea"
              v-model="rawText"
        ...
        <div v-if="activeTab === 'excel' || activeTab === 'csv'">
          ...
          <div v-if="!fileRows.length" class="clm-field">
            ...
            >
              <input
                ...
                @change="onFilePick"
              />
              <div class="clm-dz-icon"><CoolIcon :name="activeTab === 'excel' ? 'Table' : 'File_Document'" :size="28" /></div>
              ...
              <div v-if="fileError" class="clm-dz-error"><CoolIcon name="Warning" :size="14" /> {{ fileError }}</div>
            </div>
          </div>
          ...
          <div v-if="fileRows.length">
            <div class="clm-file-meta">
              <span><CoolIcon name="File_Document" :size="14" /> <b>{{ fileName }}</b></span>
            ...
            <div class="clm-field">
              ...
              <div class="clm-map-grid">
                <div class="clm-map-cell">
                  <span class="clm-map-label"><CoolIcon name="Phone" :size="13" /> SĐT <em>*</em></span>
                ...
                <div class="clm-map-cell">
                  <span class="clm-map-label"><CoolIcon name="User_01" :size="13" /> Tên KH</span>
                ...
                <div class="clm-map-cell">
                  <span class="clm-map-label"><CoolIcon name="Chat" :size="13" /> Lời mời / tin nhắn riêng</span>
        ...
        <div v-if="activeTab === 'leadads'" class="clm-leadads">
          <div class="clm-field">
            ...
            <div class="clm-platform-grid">
              ...
              >
                <CoolIcon :name="p.icon" :size="18" class="clm-platform-icon" />
          ...
          <div class="clm-field">
            <label class="clm-label">
              ...
              <span class="clm-pill" title="Mỗi tệp 1 mã. Anh đặt tên chiến dịch trên FB/TikTok kèm #MÃ — lead chảy về đúng tệp này. VD chiến dịch 'Sunshine Q7 #A-001' → tệp có mã A-001."> <CoolIcon name="Info" :size="14" /> Cách dùng </span>
            </label>
            <input
            ...
            />
            <div v-if="integrationKeyError" class="clm-err"><CoolIcon name="Warning" :size="14" /> {{ integrationKeyError }}</div>
            <div class="clm-hint">
          ...
          <label class="clm-switch">
            ...
            <span class="clm-switch-label">
              ...
              <span class="clm-pill" title="Khi bật, các lead validated trong tệp này sẽ vào pool /lead-pool/request, bất kỳ sale nào trong org có quyền nhận đều có thể click 'Nhận lead' để được assign."> <CoolIcon name="Info" :size="14" /> </span>
            </span>
          </label>

          <div class="clm-callout">
            <CoolIcon name="Bulb" :size="18" class="clm-callout-ico" />
        ...
        <div v-if="dryRunResult" class="clm-parse">
          <div class="clm-pp-row">
            <CoolIcon name="List_Checklist" :size="16" class="clm-pp-ico" /> Đã nhận diện <b>{{ dryRunResult.total }} dòng</b>
          </div>
...
const ICON_CHOICES = ['Building_01', 'Volume_Max', 'Water_Drop', 'Globe', 'List_Checklist', 'Ticket_Voucher', 'Mobile', 'Volume_Min', 'Trending_Up', 'Star'];

const TABS = [
  { key: 'paste'   as const, label: 'Paste danh sách', icon: 'List_Checklist', accept: '' },
  { key: 'excel'   as const, label: 'Upload Excel',    icon: 'Table', accept: '.xlsx,.xls' },
  { key: 'csv'     as const, label: 'Upload CSV',      icon: 'File_Document', accept: '.csv' },
  // Phase Multi-Source Lead Ads 2026-05-27
  { key: 'leadads' as const, label: 'Lead Ads',        icon: 'Volume_Max', accept: '' },
];
...
const LEAD_PLATFORMS = [
  ...
  { key: 'tiktok-leadgen', label: 'TikTok Lead Gen',   icon: 'Volume_Min' },
  { key: 'google-leadform',label: 'Google Lead Form',  icon: 'Search_Magnifying_Glass' },
  { key: 'zalo-ads',       label: 'Zalo Ads',          icon: 'Chat' },
  ...
  try {
    ...
    if (activeTab.value === 'paste') {
      ...
      result = await createList({
        ...
        iconEmoji: iconEmoji.value ?? 'Volume_Max',
        sourceType: 'leadads',
...
.clm-head {
  padding: 16px 20px;
  border-bottom: 1px solid var(--line, var(--mc-line));
...
.clm-foot {
  ...
  border-top: 1px solid var(--line, var(--mc-line));
...
.clm-input, .clm-textarea, .clm-select {
  ...
  border: 1px solid var(--line, var(--mc-line)); border-radius: 6px;
...
.clm-icon-btn {
  ...
  border: 1px solid var(--line, var(--mc-line)); border-radius: 8px;
...
.clm-file-meta {
  ...
  background: var(--surface-3, #f1f4f9); border: 1px solid var(--line, var(--mc-line));
...
.clm-preview th {
  ...
  color: var(--ink-2, #475066); border-bottom: 1px solid var(--line, var(--mc-line));
...
.clm-platform-btn {
  ...
  gap: 6px; padding: 14px 8px; border: 1px solid var(--line, var(--mc-line)); border-radius: 10px;
...
.clm-callout {
  ...
  padding: 11px 13px; border-radius: 8px; font-size: 13px; color: var(--mc-warning);
