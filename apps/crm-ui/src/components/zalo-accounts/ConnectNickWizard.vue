...
<template>
  <div class="cnw-backdrop" @click.self="onClose">
    <div class="cnw">
      <div class="cnw-head">
        <h3>{{ stepTitle }}</h3>
        <button class="cnw-x" @click="onClose"><CoolIcon name="Close_MD" :size="14" /></button>
      </div>
        ...
        <template v-else-if="step === 'done'">
          <div class="cnw-done">
            ...
            <div class="cnw-remind">
              ...
              <div>
                <b>Lưu ý quan trọng:</b> Nick này đã đăng nhập vào CRM. <b>KHÔNG</b> dùng
...
.cnw { background: var(--mc-surface); border-radius: 16px; width: 440px; max-width: 94vw; max-height: 92vh; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 12px 40px rgba(0,0,0,.25); }
.cnw-head { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px 8px; }
.cnw-head h3 { font-size: 16px; font-weight: 700; color: var(--mc-text); }
...
.cnw-step-n { width: 24px; height: 24px; border-radius: 50%; background: var(--mc-surface-alt); color: #9ca3af; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 12px; }
.cnw-step.active .cnw-step-n { background: #1786be; color: #fff; }
.cnw-step.active { color: #aeb7ff; font-weight: 600; }
.cnw-step.done .cnw-step-n { background: #12b76a; color: #fff; }

.cnw-body { padding: 8px 20px 16px; min-height: 200px; }
.cnw-field label { display: block; font-size: 13px; font-weight: 600; color: var(--mc-text); margin-bottom: 6px; }
.cnw-field input { width: 100%; padding: 11px 13px; border: 1px solid var(--mc-line); border-radius: 9px; font-size: 15px; outline: none; }
.cnw-field input:focus { border-color: #aeb7ff; }
.cnw-hint { font-size: 12px; color: #9ca3af; margin-top: 6px; }
.cnw-err { color: var(--mc-danger); font-size: 12.5px; margin-top: 8px; }
...
.cnw-avatar-ph { display: flex; align-items: center; justify-content: center; background: rgba(108,125,232,.14); color: #aeb7ff; font-weight: 700; font-size: 26px; }
.cnw-confirm-name { font-size: 17px; font-weight: 700; color: var(--mc-text); }
.cnw-confirm-phone { font-size: 14px; color: var(--mc-muted); font-variant-numeric: tabular-nums; }
.cnw-confirm-q { margin-top: 8px; font-size: 13.5px; color: var(--mc-text); }
.cnw-warn { font-size: 12.5px; color: var(--mc-warning); background: rgba(251,191,36,.14); padding: 6px 10px; border-radius: 8px; }
/* fix ①: trùng nick mình (info xanh teal Atlas v2) / trùng người khác (block đỏ) */
.cnw-info-box { display: flex; gap: 8px; align-items: flex-start; font-size: 12.5px; color: #aeb7ff; background: rgba(108,125,232,.14); border: 1px solid #bfe2f4; padding: 9px 11px; border-radius: 9px; line-height: 1.5; text-align: left; }
.cnw-block-box { display: flex; gap: 8px; align-items: flex-start; font-size: 12.5px; color: var(--mc-danger); background: rgba(248,113,113,.14); border: 1px solid #fecdca; padding: 9px 11px; border-radius: 9px; line-height: 1.5; text-align: left; }
.cnw-fallback { text-align: center; }
.cnw-fallback-msg { font-size: 14px; color: var(--mc-text); font-weight: 500; }
...
.qs { font-size: 12.5px; color: var(--mc-muted); display: flex; align-items: center; gap: 6px; }
.qs span { width: 18px; height: 18px; border-radius: 50%; background: var(--mc-surface-alt); font-size: 11px; display: inline-flex; align-items: center; justify-content: center; }
.qs.active { color: #aeb7ff; font-weight: 600; }
.cnw-scanned, .cnw-loading { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 30px; }
.cnw-spinner { width: 36px; height: 36px; border: 3px solid var(--mc-line); border-top-color: #aeb7ff; border-radius: 50%; animation: spin .8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.cnw-muted { font-size: 12.5px; color: #9ca3af; }
.cnw-qr-err { text-align: center; margin-top: 10px; }
.cnw-link { border: none; background: none; color: #aeb7ff; text-decoration: underline; cursor: pointer; font-size: 12.5px; margin-left: 6px; }
.cnw-qr-dead { text-align: center; margin-top: 12px; display: flex; flex-direction: column; align-items: center; gap: 10px; }
.cnw-qr-dead p { color: var(--mc-warning); font-size: 13.5px; margin: 0; }
...
.cnw-done h4 { font-size: 17px; font-weight: 700; color: var(--mc-success); }
.cnw-done-nick { font-size: 14px; color: var(--mc-text); }
.cnw-remind { display: flex; gap: 8px; background: rgba(251,191,36,.14); border: 1px solid #f5d9a8; border-radius: 10px; padding: 11px 13px; font-size: 12.5px; color: #7a4a08; line-height: 1.5; text-align: left; margin-top: 6px; }

.cnw-foot { display: flex; gap: 8px; justify-content: flex-end; padding: 12px 20px 18px; border-top: 1px solid #f3f4f6; }
.btn { padding: 9px 16px; border: 1px solid var(--mc-line); background: var(--mc-surface); border-radius: 9px; font-size: 13.5px; font-weight: 600; cursor: pointer; color: var(--mc-text); }
.btn-primary { background: #1786be; color: #fff; border-color: #aeb7ff; }
