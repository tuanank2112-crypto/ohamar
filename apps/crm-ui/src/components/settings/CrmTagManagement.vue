...
<template>
  <div class="crmtag-settings">
    ...
    <div class="toolbar">
      ...
      <button v-if="selected.size" class="toolbar-btn danger" @click="bulkDelete"> <CoolIcon name="Trash_Empty" :size="14" /> Xoá {{ selected.size }} tag
      </button>
      ...
      >
        ...
        <div class="name-cell">
          ...
          <template v-if="isZaloTag(tag)">
            <span class="zalo-locked" title="Tag Zalo Real — chỉ đổi/gỡ trên app Zalo, hệ thống tự sync về CRM">
              ...
              <span class="lock-icon" aria-hidden="true"><CoolIcon name="Lock" :size="14" /></span>
            </span>
          </template>
          <template v-else>
        ...
        <div class="actions-cell">
          ...
          <button class="icon-btn" :title="editingDescId === tag.id ? 'Đang sửa mô tả' : 'Sửa mô tả'"
            ...
            <span><CoolIcon name="Note_Edit" :size="14" /></span>
          </button>
          <button class="icon-btn danger" title="Xoá tag" @click="confirmDelete(tag)"><CoolIcon name="Close_MD" :size="14" /></button>
        </div>
        ...
        <div v-if="editingDescId === tag.id || tag.description" class="desc-row">
          <input v-if="editingDescId === tag.id"
            ...
            @keydown.enter="($event.target as HTMLInputElement).blur()" />
          <span v-else class="desc-text"><CoolIcon name="Note_Edit" :size="14" /> {{ tag.description }}</span>
        </div>
      </div>
    </div>
...
.toolbar-btn.danger {
  background: rgba(248,113,113,.14); color: var(--mc-danger); border-color: #ef9a9a;
}
.toolbar-btn.danger:hover { background: rgba(248,113,113,.14); color: var(--mc-danger); }
...
.icon-btn.danger:hover { background: rgba(248,113,113,.14); color: var(--mc-danger); border-color: #ef9a9a; }
