  ...
  <div class="zlbl-settings">
    ...
    <section v-for="acc in accounts" :key="acc.id" class="account-card">
      ...
      <div v-else class="empty-state">
        <p v-if="acc.status !== 'connected'"><CoolIcon name="Warning" :size="14" /> Nick này chưa kết nối — không thể đồng bộ tag.</p>
...
.acc-status.connected { color: var(--mc-success); }
.acc-status.qr_pending { color: #f57c00; }
.acc-status.disconnected { color: var(--mc-danger); }
