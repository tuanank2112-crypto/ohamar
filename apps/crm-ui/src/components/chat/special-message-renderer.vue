...
<template>
  <div class="special-message" :data-type="type" @click="onContentClick">
    ...
    <div v-else class="rich-card">
      ...
      <!-- Link -->
      <a v-if="richHref" :href="richHref" target="_blank" rel="noopener" class="rich-link"> <CoolIcon name="Link" :size="14" /> {{ richHrefLabel }}
      </a>
...
.profile-card {
  ...
  border-radius: 10px;
  background: var(--smax-grey-50, #fafbfc);
  border: 1px solid var(--smax-grey-200, var(--mc-line));
  max-width: 320px;
  position: relative;
}
...
.profile-suggest-chip {
  ...
  color: #aeb7ff; background: rgba(33, 150, 243, 0.12);
...
.profile-btn {
  ...
  border: 1px solid var(--smax-grey-300, var(--mc-line));
  background: var(--mc-surface); color: var(--smax-text);
...
.poll-status-badge.closed { color: var(--mc-muted); background: rgba(107, 114, 128, 0.15); }
.poll-status-badge.updated { color: #aeb7ff; background: rgba(33, 150, 243, 0.15); }
.poll-title {
...
.call-card.outbound-noanswer .call-title { color: var(--mc-muted); }
