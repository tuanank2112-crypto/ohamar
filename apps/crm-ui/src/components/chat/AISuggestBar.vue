...
<template>
  <div v-if="visible" class="ai-suggest-bar">
    ...
    <div class="ai-suggest-pills">
      ...
      <div v-if="!pills.length && !loading && error" class="ai-suggest-pill error"> <CoolIcon name="Warning" :size="14" /> {{ error }}
      </div>
    </div>
...
.ai-suggest-pill:hover { background: rgba(108,125,232,.14); border-color: #9c27b0; }
...
.ai-suggest-pill.error { border-color: #ef9a9a; color: var(--mc-danger); background: rgba(248,113,113,.14); }
.ai-refresh {
  width: 31px; height: 31px;
  border-radius: 50%;
