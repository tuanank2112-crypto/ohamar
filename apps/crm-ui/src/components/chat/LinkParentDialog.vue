...
<template>
  <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="500">
    <v-card>
      <v-card-title><CoolIcon name="Link" :size="14" /> Gắn KH này vào KH Cha (merge: KH hiện tại sẽ trở thành Friend của Cha)</v-card-title>
      <v-card-text>
        <v-text-field
          v-model="search"
        ...
        <div v-else class="result-list">
          ...
          >
            ...
            <div class="info">
              ...
              <div class="meta">
                <span v-if="r.phone"><CoolIcon name="Phone" :size="14" /> {{ r.phone }}</span>
                <span v-if="r.childrenCount && r.childrenCount > 0"><CoolIcon name="Users_Group" :size="14" /> {{ r.childrenCount }} con</span>
              </div>
            </div>
          </div>
        </div>
