      ...
      <template v-if="friend">
        ...
        <div class="panel-body">
          <div class="panel-section">
            <h5>Thông tin KH</h5>
            <div class="kv"><span class="k"><CoolIcon name="Mobile" :size="14" /> SĐT</span><span class="v"><b>{{ friend.contact?.phone || '—' }}</b></span></div>
            <div class="kv"><span class="k">🔵 Zalo UID</span><span class="v">{{ friend.zaloUidInNick || '—' }}</span></div>
            <div class="kv">
              <span class="k"><CoolIcon name="Calendar" :size="14" /> Là bạn từ</span>
              <span class="v">
            ...
            <div v-if="friend.contact?.province" class="kv">
              <span class="k"><CoolIcon name="Map_Pin" :size="14" /> Khu vực</span>
        ...
        <div class="panel-foot">
          <button class="btn" @click="$emit('open-chat', friend)"><CoolIcon name="Chat" :size="14" /> Mở chat</button>
          <button class="btn" @click="$emit('call', friend)"><CoolIcon name="Phone" :size="14" /> Gọi</button>
          <button class="btn primary" @click="$emit('open-contact', friend)"><CoolIcon name="User_01" :size="14" /> Hồ sơ</button>
        </div>
      </template>
    </aside>
  </Teleport>
...
.badge.success { background: var(--success-soft); color: var(--mc-success); }
.badge.warn { background: var(--warning-soft); color: var(--mc-warning); }
.badge.info { background: rgba(108,125,232,.14); color: #aeb7ff; }
.badge.grey { background: var(--surface-3); color: var(--mc-text); }
...
.tag-chip.red { background: var(--error-soft); color: var(--mc-danger); border-color: transparent; }
.tag-chip.green { background: var(--success-soft); color: var(--mc-success); border-color: transparent; }
