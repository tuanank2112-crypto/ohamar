...
<template>
  <div class="score-inline">
    ...
    <div v-else-if="!data" class="sip-empty">
      <span class="sip-empty-icon"><CoolIcon name="Chart_Bar_Vertical_01" :size="14" /></span>
    ...
    <template v-else>
      ...
      <div class="sip-dims">
        ...
        <div class="sip-dim int" :title="`Ý định mua: ${Math.round(data.intent)}/100`">
          <span class="sip-dim-icon"><CoolIcon name="Compass" :size="14" /></span>
...
.sip-trend.pos { color: var(--mc-success); background: rgba(52,211,153,.14); }
.sip-trend.neg { color: var(--mc-danger); background: rgba(248,113,113,.14); }
.sip-meta {
  font-size: 10px;
  color: var(--smax-grey-400, #a8aebb);
  margin-top: -2px;
...
.sk-line {
  background: linear-gradient(90deg,var(--mc-surface-alt) 0%,var(--mc-surface-strong) 50%,var(--mc-surface-alt) 100%);
