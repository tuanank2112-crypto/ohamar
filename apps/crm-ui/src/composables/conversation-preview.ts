...
export function latestConversationMessage<T extends ConversationPreviewMessage>(
  ...
  return messages.reduce((latest, candidate) => {
    ...
    // sentAt can be equal at millisecond precision. Zalo's snowflake is a safe tie-breaker
    // only when both messages have one; a CRM message awaiting echo must not be demoted.
...
/**
 * Keep an optimistic/socket preview only while it is newer than the server preview.
 * On equal timestamps the server wins so edits, recalls and canonical fields reconcile.
 */
