...
<template>
  <div class="settings-view">
    <header class="settings-header">
      ...
      <h1>Cài đặt chấm điểm khách hàng</h1>
    </header>

    <div v-if="loading" class="loading"><CoolIcon name="Timer" :size="14" /> Đang tải…</div>

    <div v-else-if="config" class="settings-cards">
      <!-- Weights -->
      <section class="card">
        <h2><CoolIcon name="Chart_Bar_Vertical_01" :size="14" /> Trọng số 4 chiều — tổng phải = 100%</h2>
        <div class="weight-rows">
          ...
          <div class="weight-row">
            <label><CoolIcon name="Compass" :size="14" /> Ý định mua (Intent)</label>
            <input v-model.number="weights.intent" type="number" min="0" max="100" />
            <span class="unit">%</span>
          </div>
          <div class="weight-row">
            <label><CoolIcon name="Compass" :size="14" /> Phù hợp (Fit)</label>
      ...
      <section class="card actions-card">
        <h2><CoolIcon name="Settings" :size="17" /> Hành động quản trị</h2>
        <div class="action-buttons">
          <button class="btn-primary" :disabled="saving || weightSum !== 100" @click="save">
            <CoolIcon :name="saving ? 'Loading' : 'Save'" :size="14" :class="{ 'is-spinning': saving }" /> {{ saving ? 'Đang lưu…' : 'Lưu cấu hình' }}
          </button>
          <button class="btn-secondary" :disabled="seeding" @click="seedDefaults">
            <CoolIcon :name="seeding ? 'Loading' : 'Add_Plus_Circle'" :size="14" :class="{ 'is-spinning': seeding }" /> {{ seeding ? 'Đang seed…' : 'Seed defaults (idempotent)' }}
          </button>
          <button class="btn-secondary" :disabled="recomputing" @click="recompute">
            <CoolIcon :name="recomputing ? 'Loading' : 'Arrows_Reload_01'" :size="14" :class="{ 'is-spinning': recomputing }" /> {{ recomputing ? 'Đang tính lại…' : 'Tính lại toàn bộ điểm' }}
          </button>
          <button class="btn-secondary" :disabled="scanning" @click="scanStuck">
            <CoolIcon :name="scanning ? 'Loading' : 'Search_Magnifying_Glass'" :size="14" :class="{ 'is-spinning': scanning }" /> {{ scanning ? 'Đang quét…' : 'Quét stuck ngay' }}
          </button>
        </div>
      </section>
      ...
      <section class="card">
        <h2><CoolIcon name="List_Checklist" :size="17" /> Quy tắc chấm điểm ({{ rules.length }})</h2>
...
.rules-table .pos { color: var(--mc-success); font-weight: 600; }
