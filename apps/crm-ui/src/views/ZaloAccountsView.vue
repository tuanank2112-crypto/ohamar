...
<template>
  <div class="za-page">
    ...
    <div class="za-tabs">
      ...
      > <CoolIcon name="Lock" :size="14" /> Riêng tư <span v-if="privacyCounter" class="za-tab-counter" :class="{ full: privacyCounter.atMax }">
      ...
      > <CoolIcon name="Trash_Empty" :size="14" /> Nick đã xóa </button>
    ...
    <div class="filter-row">
      <div class="search">
        <CoolIcon name="Search_Magnifying_Glass" :size="17" />
    ...
    <template v-else-if="activeTab === 'internal-contact' && !canManageZalo">
      <div class="za-locked-tab">
        <div class="za-locked-icon"><CoolIcon name="Lock" :size="14" /></div>
...
async function onWizardConfirmConnect() {
  ...
  if (!res.ok) {
    ...
    if (res.code === 'account_owned_by_other') {
      ...
      toast.push(res.message || 'Nick này đang thuộc người khác.', 'error');
    } else {
      wizardStep.value = 'phone';
      toast.push(res.message || 'Không tạo được nick. Thử lại.', 'error');
    }
    return;
  }
  ...
  else { wizardStep.value = 'phone'; toast.push('Không lấy được nick vừa tạo. Thử lại.', 'error'); }
}
...
watch(duplicateInfo, (info) => {
  if (!info) return;
  closeWizard();
  toast.push(info.message, 'warning');
...
function onTableAction(payload: { account: any; action: 'reconnect' | 'sync' }) {
  if (payload.action === 'reconnect') {
    ...
    api.post(`/zalo-accounts/${payload.account.id}/sync-contacts`)
      .then(() => refreshAll())
      .catch((e) => toast.push('Sync thất bại: ' + (e.response?.data?.error || e.message), 'error'));
  }
}
...
async function onDrawerAction(payload: { accountId: string; action: string }) {
  ...
  try {
    switch (payload.action) {
      ...
      case 'edit-proxy':
        const { ok, value: url } = await promptText({
          title: 'Cập nhật proxy',
          message: 'Để trống để xoá proxy hiện tại.',
          reasonLabel: 'Proxy URL',
          reasonPlaceholder: 'http://user:pass@host:port',
          confirmText: 'Lưu proxy',
        });
        if (!ok) return;
...
async function onRemoveCrew(payload: { accountId: string; accessId: string }) {
  ...
  try {
    ...
    toast.push('Bỏ gán thất bại: ' + (e.response?.data?.error || e.message), 'error');
  }
}
...
async function onBulkAction(action: 'reconnect' | 'sync-contacts' | 'disable') {
  ...
  try {
    ...
    if (res) {
      toast.push(`Hoàn tất: ${res.ok}/${res.total} thành công${res.failed ? `, ${res.failed} lỗi` : ''}`, res.failed ? 'warning' : 'success');
      clearSelection();
    }
  } finally {
...
// Lifecycle
// ─────────────────────────────────────────────────────────────────
// FIX U-01 (2026-07-07): store interval ref for cleanup in onUnmounted.
let __zaPollIntervalId: ReturnType<typeof setInterval> | null = null;
...
onMounted(async () => {
  ...
  __zaPollIntervalId = setInterval(() => {
    if (!document.hidden) fetchStats();
  }, 60_000);
});

// FIX U-01 (2026-07-07): clear polling interval when leaving page — prevents stacking.
onUnmounted(() => {
  if (__zaPollIntervalId !== null) {
    clearInterval(__zaPollIntervalId);
    __zaPollIntervalId = null;
  }
});
</script>
...
.za-tab:hover { color: var(--mc-text); }
.za-tab.active { color: #aeb7ff; border-bottom-color: #aeb7ff; }
.za-tab-counter {
  font-size: 11px; font-weight: 700;
  padding: 2px 8px; border-radius: 9999px;
  background: rgba(108,125,232,.14); color: #aeb7ff;
  font-variant-numeric: tabular-nums;
}
.za-tab-counter.full { background: rgba(248,113,113,.14); color: var(--mc-danger); }

/* 2026-06-09 — sub-tab Đơn giản / Nâng cao (cấp 2): pill segmented, cùng tông brand Atlas v2. */
.za-subtabs {
  display: inline-flex; gap: 3px; padding: 3px;
  background: var(--mc-surface-alt); border-radius: 10px;
  margin-bottom: 16px;
}
.za-subtab {
  background: transparent; border: none; cursor: pointer;
  padding: 7px 16px; font-family: inherit; font-size: 13px; font-weight: 600;
  color: var(--mc-muted); border-radius: 7px;
  display: inline-flex; align-items: center; gap: 6px;
  transition: color .15s, background .15s, box-shadow .15s;
}
.za-subtab:hover { color: var(--mc-text); }
.za-subtab.active {
  color: #aeb7ff; background: var(--mc-surface);
...
.za-groupby-lbl { font-size: 12.5px; color: var(--mc-muted); font-weight: 600; margin-right: 2px; }
.za-groupby-opt {
  ...
  color: var(--mc-muted); border-radius: 8px; transition: all .15s;
}
.za-groupby-opt:hover { border-color: #c7d2fe; color: var(--mc-text); }
.za-groupby-opt.active { background: rgba(108,125,232,.14); border-color: #aeb7ff; color: #aeb7ff; }

/* Phase 4 redesign 2026-05-22: filter chip Phòng ban + group-by toggle */
.chip-multi { position: relative; }
.chip-btn {
  display: inline-flex; align-items: center; gap: 6px;
  padding: 7px 12px; background: var(--mc-surface); border: 1px solid var(--mc-line);
  border-radius: 6px; font-size: 13px; cursor: pointer;
  font-family: inherit; color: var(--mc-text);
}
.chip-btn:hover { background: var(--mc-surface-alt); border-color: #C7CCEB; }
.chip-multi.open .chip-btn { border-color: #aeb7ff; background: rgba(108,125,232,.14); color: #aeb7ff; }
.chip-count {
...
.chip-pop {
  ...
  background: var(--mc-surface); border: 1px solid var(--mc-line); border-radius: 8px;
...
.chip-pop-head {
  ...
  font-size: 11px; color: var(--mc-muted); text-transform: uppercase; letter-spacing: .04em; font-weight: 600;
}
.chip-clear {
  background: transparent; border: none; color: #aeb7ff;
...
.chip-pop-row {
  ...
  padding: 8px 12px; font-size: 13px; color: var(--mc-text);
  cursor: pointer;
}
.chip-pop-row:hover { background: var(--mc-surface-alt); }
.chip-pop-empty { padding: 16px; text-align: center; color: #9CA3AF; font-size: 12px; }

.toggle-group {
  display: inline-flex; align-items: center; gap: 6px;
  padding: 7px 10px; background: var(--mc-surface); border: 1px solid var(--mc-line);
  border-radius: 6px; font-size: 12.5px; color: var(--mc-text);
  cursor: pointer; user-select: none;
}
.toggle-group input { cursor: pointer; accent-color: #aeb7ff; }
.toggle-group:has(input:checked) { background: rgba(108,125,232,.14); border-color: #aeb7ff; color: #aeb7ff; }
...
.topbar .sub b { color: var(--mc-ink); font-weight: 600 }
.topbar .sub .warn { color: var(--mc-danger); font-weight: 500 }
.topbar .sub .dot { margin: 0 6px; color: var(--mc-line) }
...
.search svg { width: 13px; height: 13px; color: var(--mc-muted) }
...
.select:hover { border-color: var(--mc-line) }
...
.modal-head h3 { margin: 0; font-size: 15px; font-weight: 600; color: var(--mc-ink) }
.x-btn {
  background: transparent;
  border: none;
  color: var(--mc-muted);
...
.qr-scanned p { color: var(--mc-success); font-weight: 500; margin: 8px 0 }
.qr-scanned .muted { color: var(--mc-muted); font-weight: 400; font-size: 12px }
.error-text {
  color: var(--mc-danger);
  font-size: 12px;
  margin-top: 8px;
