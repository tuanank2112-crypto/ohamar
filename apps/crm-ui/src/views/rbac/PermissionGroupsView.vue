...
<template>
  <div class="dept-page">
    <header class="page-hero">
      ...
      <div class="hero-actions">
        <button class="btn-ghost" :disabled="seeding" @click="seedDefaults">
          <CoolIcon :name="seeding ? 'Loading' : 'Settings'" :size="14" :class="{ 'is-spinning': seeding }" /> {{ seeding ? 'Đang seed...' : 'Seed 7 nhóm mặc định' }}
        </button>
        <button class="btn-primary" @click="openCreate(null)">
          <CoolIcon name="Add_Plus" :size="14" /> Thêm nhóm
        </button>
      </div>
    </header>
    ...
    <div v-else-if="store.permissionGroups.length === 0" class="empty-state">
      <div class="empty-icon"><CoolIcon name="Shield" :size="32" /></div>
      ...
      <aside class="pg-sidebar">
        <div class="pg-sidebar-head">
          <div class="search-box at-search">
            ...
            <button v-if="searchQ" class="search-clear" @click="searchQ = ''"><CoolIcon name="Close_MD" :size="13" /></button>
          </div>
        </div>
          ...
          >
            ...
            <div class="pg-group-body">
              ...
              <div class="pg-group-meta">
                ...
                <span v-else class="at-chip chip-custom chip-xs"><CoolIcon name="Edit_Pencil_01" :size="14" /> Tùy chỉnh</span>
                <span class="pg-count"><CoolIcon name="Users_Group" :size="14" /> {{ memberCountsLive[g.id] ?? 0 }}</span>
      ...
      <section class="pg-main">
        ...
        <template v-else>
            ...
            <div class="pg-matrix-title">
              ...
              <div>
                ...
                <div class="pg-name-meta">
                  ...
                  <span v-else class="at-chip chip-custom"><CoolIcon name="Edit_Pencil_01" :size="14" /> Tùy chỉnh</span>
                  <span class="at-chip chip-dept"><CoolIcon name="Users_Group" :size="14" /> {{ memberCountsLive[selected.id] ?? 0 }} user</span>
          ...
          <div class="pg-save-bar" :class="{ 'is-saving': saving, 'is-saved': justSaved }">
            ...
            <span v-else-if="justSaved"><CoolIcon name="Check" :size="14" /> Đã lưu</span>
          ...
          <div v-if="!selected.isSystem" class="pg-danger-zone">
            ...
            <button class="btn-danger" @click="confirmArchive"><CoolIcon name="Trash_Empty" :size="14" /> Xóa nhóm</button>
          </div>
        </template>
      </section>
    </div>
...
async function tickAll(value: boolean) {
  if (!selected.value) return;
  const label = value ? 'Tick TẤT CẢ' : 'Bỏ tick TẤT CẢ';
  if (!(await confirm({ title: `${label} quyền cho nhóm "${selected.value.name}"?`, tone: value ? 'primary' : 'danger' }))) return;
...
async function doCopyFrom() {
  ...
  if (!src) return;
  if (!(await confirm({ title: `Sao chép quyền từ "${src.name}"?`, message: `Sẽ ghi đè quyền hiện tại của "${selected.value.name}".`, tone: 'danger', confirmText: 'Ghi đè' }))) return;
...
async function confirmArchive() {
  if (!selected.value) return;
  if (!(await confirm({ title: `Xóa nhóm "${selected.value.name}"?`, message: 'Chỉ xóa được khi nhóm rỗng.', tone: 'danger', confirmText: 'Xóa' }))) return;
  try {
...
.pg-group-item:hover { background: var(--mc-surface-alt); border-color: var(--mc-line); }
...
.pg-count { font-size: 10px; color: var(--mc-text); font-weight: 500; }
...
.pg-add-btn:hover { border-color: var(--mc-ink); color: var(--mc-ink); background: var(--mc-surface-alt); }
...
.th-bulk-btn:hover { background: #181d26; color: white; border-color: var(--mc-ink); }
.pg-matrix thead .th-row-bulk { min-width: 70px; }

.pg-matrix tbody tr {
  transition: background 0.1s;
}
.pg-matrix tbody tr:hover { background: var(--mc-surface-alt); }
.pg-matrix tbody tr.row-full { background: rgba(52,211,153,.14); }
.pg-matrix tbody tr.row-full:hover { background: rgba(52,211,153,.14); }
.pg-matrix tbody tr.row-empty .cell-resource { color: var(--mc-muted); }
...
.pg-matrix tbody tr:hover .cell-resource { background: var(--mc-surface-alt); }
.pg-matrix tbody tr.row-full .cell-resource { background: rgba(52,211,153,.14); }
.pg-matrix tbody tr.row-full:hover .cell-resource { background: rgba(52,211,153,.14); }
...
.at-checkbox:hover .at-checkbox-box { border-color: var(--mc-muted); }
...
.pg-save-bar.is-saving { color: #aeb7ff; background: rgba(108,125,232,.14); }
.pg-save-bar.is-saved { color: var(--mc-success); background: rgba(52,211,153,.14); }
.pg-save-hint { color: var(--mc-muted); font-style: italic; }
...
.pg-danger-zone strong { color: var(--mc-danger); font-size: 13px; }
.pg-danger-hint { font-size: 11px; color: var(--mc-text); margin: 4px 0 0; }
.btn-danger {
  background: var(--mc-surface);
