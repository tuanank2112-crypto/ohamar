  ...
  <div class="airtable-scope apt-page">
    ...
    <div class="apt-filter-strip">
      ...
      <span v-if="source !== 'all'" class="at-chip" @click="source = 'all'">
        Nguồn: {{ source === 'zalo' ? 'Zalo' : 'Thủ công' }} <span class="x"><CoolIcon name="Close_MD" :size="14" /></span>
      </span>
...
.at-segmented button:hover:not(.active):not(:disabled) { background: rgba(255,255,255,.045); color: var(--at-ink); }
.at-segmented button.active {
  background: var(--at-action-soft);
  color: var(--at-action-text, var(--at-action));
  box-shadow: inset 0 0 0 1px rgba(108,125,232,.32);
}
.at-segmented button:focus-visible,
.filter-trigger:focus-visible,
.at-chip:focus-visible,
.icon-btn:focus-visible {
  outline: 2px solid var(--at-action);
  outline-offset: 2px;
}
...
.filter-trigger:hover { background: var(--at-surface-soft); color: var(--at-ink); }
.filter-trigger:active { background: var(--at-surface-strong); }
.filter-trigger.active {
  border-color: rgba(108,125,232,.48);
  background: var(--at-action-soft);
  color: var(--at-action-text, var(--at-action));
}
...
.filter-trigger.active .trigger-count {
  background: rgba(108,125,232,.24);
  color: var(--at-action-text, var(--at-action));
}
...
:global(.filter-menu .menu-pill.status-scheduled) { background: rgba(251,191,36,.14); color: var(--mc-warning); }
:global(.filter-menu .menu-pill.status-overdue)   { background: rgba(251,191,36,.14); color: var(--mc-warning); }
:global(.filter-menu .menu-pill.status-completed) { background: rgba(52,211,153,.14); color: var(--mc-success); }
:global(.filter-menu .menu-pill.status-cancelled) { background: var(--mc-line); color: var(--mc-text); text-decoration: line-through; }
:global(.filter-menu .menu-pill.status-no_show)   { background: rgba(248,113,113,.14); color: var(--mc-danger); }
:global(.filter-menu .menu-pill.type)             { background: var(--mc-surface-alt); color: var(--mc-text); border: 1px solid var(--mc-line); }
...
.at-chip:hover { background: var(--at-surface-soft); color: var(--at-ink); }
.at-chip:active { background: var(--at-surface-strong); }
.at-chip--active {
  background: var(--at-action-soft);
  color: var(--at-action-text, var(--at-action));
