...
<template>
  <div style="max-width: 700px;">
    <h1 class="text-h5 mb-4">
      <v-icon class="mr-2" style="color: var(--mc-cyan);">mdi-api</v-icon>
      API & Webhook
    </h1>
...
async function copyKey() {
  ...
  const ok = await copy(apiKey.value);
  showSnack(ok ? 'Đã sao chép API key' : 'Copy thất bại — bôi đen + Ctrl+C', ok ? undefined : 'error');
}
