...
<template>
  <div class="airtable-scope dh-v4">
    ...
    <div class="at-dash-body">
      <!-- ════════════════ TAB 1 — VIỆC CỦA TÔI ════════════════ -->
      <div v-show="activeTab === 'me'" class="dh-tabpanel dh-workspace">
       <!-- ── MAIN PANEL ── -->
       <div class="dh-main">
        <!-- Greeting -->
        <div class="at-greet">
          ...
          <div class="at-greet__r">
            ...
            <div v-if="canPickUser" class="dh-scope" style="position:relative">
              <button class="at-roletabs__scope" @click.stop="userPickerOpen = !userPickerOpen">
                ...
                <span class="scope-label">{{ currentViewedUserName }}</span>
        ...
        <div class="at-card">
              ...
              <div class="at-card__body">
                ...
                <div v-if="!me?.urgent.length" class="at-empty at-empty--illustrated">
                  <span class="at-empty__icon"><Inbox :size="20" :stroke-width="1.7" /></span>
                  <div class="at-empty__title">Hộp thư đã được xử lý</div>
                  <div class="at-empty__copy">Không có tin nhắn nào đang chờ bạn phản hồi.</div>
                </div>
              </div>
            </div>
       ...
       <aside class="dh-rail">
         <!-- Breadcrumb -->
         <div class="dh-rail__crumb">
           <span>Tổng quan</span><span class="dh-rail__crumb-sep">/</span>
           <b>Hôm nay</b><span class="dh-rail__crumb-dot"></span>
         </div>
         ...
         <div class="at-card">
           ...
           <div class="at-card__body">
             ...
             <div v-if="reminderCount === 0" class="at-empty at-empty--illustrated at-empty--compact">
               <span class="at-empty__icon"><Bell :size="18" :stroke-width="1.7" /></span>
               <div class="at-empty__title">Lịch đang trống</div>
               <div class="at-empty__copy">Bạn chưa có nhắc nhở gần đây.</div>
             </div>
           </div>
         </div>
           ...
           <div class="at-quota">
             <div v-for="n in me?.quotaNicks ?? []" :key="n.id" class="at-quota__line">
               ...
               <div class="at-bar">
                 ...
                 <div v-else class="at-bar__seg at-bar__seg--priv"></div>
               </div>
             </div>
         ...
         <!-- Danh sách công việc (local, lưu localStorage) -->
         <div class="dh-tasks">
           <div class="dh-tasks__head">
             <div class="at-card__title"><ListTodo :size="14" :stroke-width="2" /> Danh sách công việc</div>
             <span class="dh-tasks__count">{{ tasks.filter(t => t.done).length }}/{{ tasks.length }}</span>
           </div>
           <div
             v-for="t in tasks"
             :key="t.id"
           ...
           >
             <button class="dh-task__check" :class="{ on: t.done }" @click="toggleTask(t.id)">
               <Check v-if="t.done" :size="11" :stroke-width="3" />
             </button>
             <span class="dh-task__label">{{ t.label }}</span>
             <button class="dh-task__del" :aria-label="`Xóa công việc ${t.label}`" @click="removeTask(t.id)"><Trash2 :size="13" :stroke-width="2" /></button>
           </div>
           <div class="dh-task dh-task--input">
             <Circle :size="14" :stroke-width="2" class="dh-task__circle" />
             <input
               v-model="newTask"
               placeholder="Nhập công việc mới..."
               @keydown.enter="addTask"
             />
             <div class="dh-task__tools"><Calendar :size="13" :stroke-width="2" /><Paperclip :size="13" :stroke-width="2" /></div>
           </div>
         </div>

         <!-- Ghi chú nhanh -->
         <div class="dh-note">
           <div class="at-card__title"><StickyNote :size="14" :stroke-width="2" /> Ghi chú nhanh</div>
           <textarea v-model="quickNote" placeholder="Viết ghi chú cho hôm nay..." rows="3"></textarea>
         </div>
       </aside>
      </div>

      <!-- ════════════════ TAB 2 — QUẢN LÝ TEAM ════════════════ -->
      <div v-show="activeTab === 'team'" class="dh-tabpanel dh-section-layout">
      ...
      <div v-show="activeTab === 'system'" class="dh-tabpanel dh-section-layout">
...
import {
  ...
  Sparkles, Circle, Activity, Trophy, Check, Trash2, ListTodo,
  StickyNote, Calendar, Paperclip,
...
// ── Right-rail: local task list + quick note (persist localStorage) ──
interface DashTask { id: number; label: string; done: boolean }
const TASKS_KEY = 'dh.tasks';
const NOTE_KEY = 'dh.note';
function loadTasks(): DashTask[] {
  try { return JSON.parse(localStorage.getItem(TASKS_KEY) || '[]'); } catch { return []; }
}
const tasks = ref<DashTask[]>(loadTasks());
const newTask = ref('');
const quickNote = ref(localStorage.getItem(NOTE_KEY) || '');
function persistTasks() { localStorage.setItem(TASKS_KEY, JSON.stringify(tasks.value)); }
function addTask() {
  const v = newTask.value.trim();
  if (!v) return;
  tasks.value.push({ id: Date.now(), label: v, done: false });
  newTask.value = '';
  persistTasks();
}
function toggleTask(id: number) {
  const t = tasks.value.find((x) => x.id === id);
  if (t) { t.done = !t.done; persistTasks(); }
}
function removeTask(id: number) {
  tasks.value = tasks.value.filter((x) => x.id !== id);
  persistTasks();
}
watch(quickNote, (v) => localStorage.setItem(NOTE_KEY, v));
...
const funnelBars = computed(() => {
  ...
  return f
    ...
    .slice(0, 6)
    .map((x) => ({ status: x.status ?? 'khac', label: STATUS_MAP[x.status ?? '']?.label ?? x.status ?? 'Khác', count: x.count, pct: Math.round((x.count / max) * 100), color: FUNNEL_COLOR[x.status ?? ''] ?? 'var(--mc-line)' }));
});
...
.dh-pdd {
  ...
  background: var(--at-canvas, #fff); border: 1px solid var(--at-hairline, var(--mc-line));
  border-radius: 9px; box-shadow: 0 12px 32px rgba(2,6,23,0.55); padding: 6px;
}
