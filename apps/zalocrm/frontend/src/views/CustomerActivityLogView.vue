  ...
  <div class="al-view">
    <header class="al-header">
      ...
      <div class="al-actions">
        <button class="export-btn" :disabled="exporting" @click="exportCsv">
          <CoolIcon :name="exporting ? 'Loading' : 'Download'" :size="14" :class="{ 'is-spinning': exporting }" /> {{ exporting ? 'Đang xuất…' : 'Xuất CSV' }}
        </button>
      </div>
    </header>
    ...
    <div class="al-layout">
      ...
      <aside class="al-filters" :class="{ open: filterDrawerOpen }">
        ...
        <section class="filter-block">
          <h3><CoolIcon name="Tag" :size="14" /> Loại hoạt động</h3>
          <div class="cat-list">
            <label
            ...
            >
              ...
              />
              <CoolIcon :name="CATEGORY_META[cat].icon" :size="16" class="cat-icon" :style="`color: ${CATEGORY_META[cat].color}`" />
        ...
        <section class="filter-block">
          <h3><CoolIcon name="User_01" :size="14" /> Người thực hiện</h3>
          <label class="actor-row">
            <input type="checkbox" :checked="actorTypes.has('user')" @change="toggleActorType('user')" />
            <span><CoolIcon name="User_01" :size="14" /> Người dùng (sale/admin)</span>
          </label>
          <label class="actor-row">
            <input type="checkbox" :checked="actorTypes.has('bot')" @change="toggleActorType('bot')" />
            <span><CoolIcon name="Settings_Future" :size="14" /> Bot (automation)</span>
          </label>
          <label class="actor-row">
            <input type="checkbox" :checked="actorTypes.has('system')" @change="toggleActorType('system')" />
            <span><CoolIcon name="Settings" :size="14" /> Hệ thống</span>
          </label>
        </section>

        <section class="filter-block">
          <h3><CoolIcon name="Search_Magnifying_Glass" :size="14" /> Tìm kiếm</h3>
          <input
            type="text"
            v-model="search"
      ...
      <!-- Main list -->
      <main class="al-main">
        <button class="filter-toggle-mobile" @click="filterDrawerOpen = true"> <CoolIcon name="Search_Magnifying_Glass" :size="14" /> Bộ lọc <span v-if="activeFilterCount" class="filter-count-badge">{{ activeFilterCount }}</span>
        </button>
