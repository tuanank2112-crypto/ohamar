  ...
  <div class="roles-page">
      ...
      <div class="roles-grid">
        <article v-for="role in SYSTEM_ROLES" :key="role.id" class="role-card" :class="`role-${role.id}`">
          <header class="rc-head">
            <div class="rc-badge">
              <CoolIcon :name="role.icon" :size="18" class="rc-icon" />
    ...
    <section class="perm-section">
        ...
        <table class="perm-table">
          <thead>
            <tr>
              ...
              <th v-for="role in SYSTEM_ROLES" :key="role.id" class="th-role">
                <CoolIcon :name="role.icon" :size="16" class="th-role-icon" />
                {{ role.label }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="module in PERMISSION_MATRIX" :key="module.id">
              <td class="td-module">
                <CoolIcon :name="module.icon" :size="16" class="td-module-icon" />
                <div>
      ...
      <div class="perm-legend">
        ...
        <span class="legend-item"><span class="perm-icon own"><CoolIcon name="User_01" :size="14" /></span> Own only</span>
        <span class="legend-item"><span class="perm-icon none"><CoolIcon name="Close_MD" :size="14" /></span> No access</span>
      </div>
    </section>
...
.rc-detail-btn:hover { background: rgba(108,125,232,.14); border-color: #aeb7ff; }
...
.perm-icon.full { background: rgba(52,211,153,.14); color: var(--mc-success); }
.perm-icon.read { background: rgba(108,125,232,.14); color: #aeb7ff; }
.perm-icon.own { background: rgba(251,191,36,.14); color: var(--mc-warning); }
.perm-icon.none { background: var(--mc-surface-alt); color: var(--mc-muted); }
.perm-label {
  font-size: 11.5px;
  font-weight: 500;
  color: var(--mc-muted);
