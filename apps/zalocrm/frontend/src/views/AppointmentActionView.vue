...
<template>
  <div class="aa-wrap">
    <div class="aa-card">
      <div class="aa-brand"><img class="aa-logo" src="/brand/monarch-logo.png" alt="Monarch CRM" /></div>
      ...
      <template v-else-if="appt">
        <div class="aa-head">
          <div class="aa-ic cal"><CoolIcon name="Calendar" :size="14" /></div>
        ...
        <div v-else class="aa-actions">
          ...
          <button class="aa-btn cancel" :disabled="busy" @click="act('cancelled')"><CoolIcon name="Close_MD" :size="14" /> Huỷ lịch</button>
        </div>
      </template>
    </div>
  </div>
...
.aa-wrap { position: fixed; inset: 0; box-sizing: border-box;
  ...
  background: radial-gradient(circle at 10% 10%,rgba(34,211,238,.15),transparent 32%),radial-gradient(circle at 90% 90%,rgba(79,70,229,.16),transparent 35%),#f7f8fc; }
.aa-card { width: 100%; max-width: 380px; margin: auto; background: var(--mc-surface); border-radius: 16px; padding: 22px 20px 24px;
  box-shadow: 0 16px 48px rgba(0,0,0,.3); font-family: Inter, system-ui, -apple-system, sans-serif; color: var(--mc-text); }
.aa-brand { display: flex; align-items: center; gap: 8px; font-weight: 800; font-size: 14px; color: var(--mc-ink); margin-bottom: 16px; }
.aa-logo { display: block; width: 132px; height: auto; }
...
.aa-ic.cal { background: rgba(108,125,232,.14); }
.aa-ic.ok { background: rgba(52,211,153,.14); color: #12b76a; font-weight: 800; }
.aa-ic.err { background: rgba(248,113,113,.14); color: #f04438; font-weight: 800; }
...
.aa-row .l { color: var(--mc-text); }
.aa-row .v { font-weight: 600; }
.aa-badge { font-size: 12px; font-weight: 700; padding: 2px 9px; border-radius: 999px; background: var(--mc-surface-alt); color: var(--mc-text); }
.aa-badge.completed { background: rgba(52,211,153,.14); color: var(--mc-success); }
.aa-badge.cancelled, .aa-badge.no_show { background: rgba(248,113,113,.14); color: var(--mc-danger); }
.aa-badge.overdue { background: rgba(251,191,36,.14); color: var(--mc-warning); }
.aa-q { font-size: 13px; color: var(--mc-text); margin: 0 0 10px; font-weight: 600; }
...
.aa-btn.ok { background: linear-gradient(135deg,#2563eb,#4f46e5); color: #fff; }
.aa-btn.cancel { background: var(--mc-surface); color: var(--mc-danger); border: 1px solid #f0c0bb; }
...
.aa-sub { color: var(--mc-text); font-size: 13px; margin: 0; }
.aa-err p { color: var(--mc-danger); font-weight: 600; }
</style>
