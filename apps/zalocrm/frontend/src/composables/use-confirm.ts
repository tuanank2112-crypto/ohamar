...
export interface ConfirmOptions {
  ...
  /** Hiện ô nhập văn bản dùng chung (không nhất thiết là lý do). */
...
export function useConfirm() {
  ...
  /** Hộp nhập văn bản đồng bộ giao diện, thay window.prompt(). */
  function promptText(opts: ConfirmOptions): Promise<{ ok: boolean; value: string }> {
    if (pending) resolveConfirm(false, '');
    return new Promise<{ ok: boolean; value: string }>((resolve) => {
      pending = (r) => resolve({ ok: r.ok, value: r.reason });
      confirmState.value = { ...opts, tone: opts.tone ?? 'primary', collectInput: true, open: true };
    });
  }

  return { confirm, confirmWithReason, promptText };
}
