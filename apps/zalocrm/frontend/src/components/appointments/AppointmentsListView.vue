...
<template>
  <div class="apt-list">
    <div v-if="!appointments.length" class="empty">
      <div class="empty-icon"><CoolIcon name="Calendar" :size="14" /></div>
    ...
    <div v-for="group in grouped" :key="group.iso" class="group">
      <h3>
        {{ group.label }}
        <span class="badge">{{ group.items.length }} lịch<span v-if="group.conflicts"> · <CoolIcon name="Warning" :size="14" /> {{ group.conflicts }} trùng giờ</span></span>
      </h3>
      <div
      ...
      >
        ...
        <div class="col col-customer">
          ...
          <div class="info">
            ...
            <div class="sub">
              <span v-if="a.contact?.phone"><CoolIcon name="Mobile" :size="14" /> {{ a.contact.phone }}</span>
            </div>
...
.empty-btn:hover { background: var(--at-primary-active); }
.empty-btn:focus-visible { outline: 2px solid var(--at-action); outline-offset: 2px; }
...
.row.done .col-time { text-decoration: line-through; color: var(--mc-muted); }
...
.pill.status-scheduled { background: rgba(251,191,36,.14); color: var(--mc-warning); }    /* peach tint */
.pill.status-overdue   { background: rgba(251,191,36,.14); color: var(--mc-warning); }    /* mustard tint */
.pill.status-completed { background: rgba(52,211,153,.14); color: var(--mc-success); }    /* forest tint */
.pill.status-cancelled { background: var(--at-surface-strong); color: var(--at-muted); text-decoration: line-through; }
.pill.status-no_show   { background: rgba(248,113,113,.14); color: var(--mc-danger); }    /* coral tint */
