    ...
    <div class="apt-panel-inner">
      <template v-if="appointment">
        <div class="panel-head">
          ...
          <button class="close" @click="$emit('close')"><CoolIcon name="Close_MD" :size="14" /></button>
        </div>
        ...
        <div class="panel-body">
          <div class="panel-section">
            ...
            <div class="cust-card">
              ...
              <div class="info">
                ...
                <div class="sub">
                  <span v-if="appointment.contact?.phone"><CoolIcon name="Mobile" :size="14" /> {{ appointment.contact.phone }}</span>
              ...
              <div class="actions-stack">
                ...
                ><CoolIcon name="Chat" :size="14" /></button>
                <button
                  v-if="appointment.contact?.id"
                  title="Hồ sơ KH"
                  @click="$emit('open-contact', appointment)"
                ><CoolIcon name="User_01" :size="14" /></button>
              </div>
            </div>
          </div>
          ...
          <div class="panel-section">
            ...
            <div class="kv-row">
              <span class="k"><CoolIcon name="Link" :size="14" /> Nguồn</span>
              <span class="v">
            ...
            <div v-if="appointment.notes" class="kv-row">
              <span class="k"><CoolIcon name="Note_Edit" :size="14" /> Ghi chú</span>
          ...
          <div class="panel-section">
            ...
            <div class="timeline">
              <div v-if="appointment.statusChangedAt && appointment.statusChangedBy" class="tl-item done">
                ...
                <div class="what"><CoolIcon name="Check" :size="14" /> {{ statusLabel(appointment.status) }} — bởi {{ appointment.statusChangedBy.fullName || appointment.statusChangedBy.email }}</div>
              </div>
              <div class="tl-item done">
                <div class="when">{{ formatRelative(appointment.createdAt) }}</div>
                <div class="what"><CoolIcon name="Add_Plus_Circle" :size="14" /> Lịch hẹn được tạo</div>
        ...
        <div class="panel-foot">
          <button class="btn" @click="$emit('reschedule', appointment)"><CoolIcon name="Calendar" :size="14" /> Đổi giờ</button>
          <button
            v-if="appointment.status === 'scheduled' || appointment.status === 'overdue'"
            class="btn danger"
            @click="$emit('cancel', appointment)"
          ><CoolIcon name="Close_MD" :size="14" /> Huỷ</button>
          <button
            v-if="appointment.status === 'scheduled' || appointment.status === 'overdue'"
            class="btn warn"
            @click="$emit('no-show', appointment)"
          ><CoolIcon name="User_01" :size="14" /> Vắng</button>
          <button
            v-if="appointment.status === 'scheduled' || appointment.status === 'overdue'"
            class="btn primary"
...
.pill.status-scheduled { background: rgba(251,191,36,.14); color: var(--mc-warning); }
.pill.status-overdue   { background: rgba(251,191,36,.14); color: var(--mc-warning); }
.pill.status-completed { background: rgba(52,211,153,.14); color: var(--mc-success); }
.pill.status-cancelled { background: var(--at-surface-strong); color: var(--at-muted); text-decoration: line-through; }
.pill.status-no_show   { background: rgba(248,113,113,.14); color: var(--mc-danger); }
...
.panel-foot .btn.danger:active { background: rgba(248,113,113,.14); }
.panel-foot .btn.warn { color: var(--mc-warning); border-color: var(--at-mustard); }
.panel-foot .btn.warn:active { background: rgba(251,191,36,.14); }
</style>
