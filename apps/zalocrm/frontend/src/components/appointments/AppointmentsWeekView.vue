      ...
      <div v-for="d in days" :key="d.iso" class="daycol" :class="{ today: d.isToday }">
        ...
        >
          ...
          <template v-else>
            ...
            <div class="ev-meta">
              <span v-if="ev.appt.location" class="ev-loc"><CoolIcon name="Map_Pin" :size="14" /> {{ ev.appt.location }}</span>
...
.cal-head {
  ...
  /* Thân lịch có scrollbar dọc nên content hẹp hơn header. Ép header giữ cùng
     scrollbar gutter để đường dọc 7 ngày không lệch tăng dần về phía CN. */
  overflow-y: scroll;
  scrollbar-gutter: stable;
  scrollbar-width: thin;
  scrollbar-color: transparent transparent;
}
.cal-head::-webkit-scrollbar { width: 10px; }
.cal-head::-webkit-scrollbar-track,
.cal-head::-webkit-scrollbar-thumb { background: transparent; }
...
.cal-head .dcol.today .dow { color: var(--at-action-text, var(--at-action)); }
...
.daycol .slot {
  height: 48px;
  border-bottom: 1px solid var(--at-hairline);
  /* Khớp timecol: border phải nằm trong 48px. Nếu để content-box, mỗi hàng
     bên ngày thành 49px và đường kẻ lệch tích lũy theo từng giờ. */
  box-sizing: border-box;
  cursor: pointer;
  position: relative;
}
.daycol .slot:hover { background: rgba(108,125,232,.055); }
.daycol .slot:active { background: var(--at-action-soft); }
...
.daycol .slot .halfslot:active { background: rgba(108,125,232,.08); }
.daycol.today { background: rgba(108,125,232,.035); }
