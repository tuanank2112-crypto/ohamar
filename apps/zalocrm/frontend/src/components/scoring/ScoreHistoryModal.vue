  ...
  <Teleport to="body">
    <div v-if="modelValue" class="shm-overlay" @click.self="close">
      <div class="shm-card" role="dialog" aria-modal="true">
        ...
        <div v-if="loading" class="shm-loading"><CoolIcon name="Timer" :size="14" /> Đang tải lịch sử…</div>
...
.shm-row-dim.dim-engagement { color: #aeb7ff; background: rgba(108,125,232,.14); }
.shm-row-dim.dim-intent { color: var(--mc-success); background: rgba(52,211,153,.14); }
.shm-row-dim.dim-fit { color: #c4b5fd; background: rgba(108,125,232,.14); }
.shm-row-dim.dim-velocity { color: var(--mc-warning); background: rgba(251,191,36,.14); }
