  ...
  <div class="ic-page">
    ...
    <template v-else-if="state.status === 'ready'">
      <header class="ic-done-head">
        <div class="ic-done-badge"><CoolIcon name="Check" :size="14" /></div>
        <div>
      ...
      <div v-if="resetError" class="ic-error"><CoolIcon name="Warning" :size="14" /> {{ resetError }}</div>
    </template>
      ...
      <div class="ic-cards">
        <button class="ic-card" @click="wizardMethod = 'crm_nick'">
          <div class="ic-card-head">
            <span class="ic-card-icon"><CoolIcon name="Mobile" :size="14" /></span>
          ...
          <ul class="ic-card-pros">
            <li><CoolIcon name="Check" :size="14" /> Setup 1 phút</li>
            <li><CoolIcon name="Check" :size="14" /> Không tốn nick mới</li>
            <li><CoolIcon name="Check" :size="14" /> Thông báo + chat khách dùng chung 1 app Zalo</li>
          </ul>
        ...
        <button class="ic-card" @click="wizardMethod = 'personal_phone'">
          ...
          <ul class="ic-card-pros">
            <li><CoolIcon name="Check" :size="14" /> Riêng tư hơn</li>
            <li><CoolIcon name="Check" :size="14" /> Nick cá nhân không bị CRM động</li>
            <li><CoolIcon name="Warning" :size="14" /> Cần SĐT có Zalo</li>
          </ul>
          <span class="ic-card-cta">Chọn cách này →</span>
        </button>
      </div>
      ...
      <div v-else class="ic-nick-list">
        ...
        >
          ...
          <div class="ic-nick-info">
            ...
            <div class="ic-nick-meta"> <CoolIcon name="Users_Group" :size="14" /> {{ n.friendCount || 0 }} bạn<template v-if="n.phone"> · {{ n.phone }}</template>
            </div>
          </div>
        </label>
      </div>

      <p class="ic-tip"><CoolIcon name="Bulb" :size="14" /> Tip: chọn nick bạn check Zalo thường xuyên nhất</p>

      <div v-if="initError" class="ic-error"><CoolIcon name="Warning" :size="14" /> {{ initError }}</div>

      <div class="ic-step-actions">
        <button class="ic-btn-primary" :disabled="!pickedNickId || initing" @click="onInitiateCrmNick">
          <span v-if="initing"><CoolIcon name="Timer" :size="14" /> Đang gửi lời mời...</span>
    ...
    <template v-else-if="wizardMethod === 'personal_phone' && !state.method">
      ...
      <div class="ic-warn-box"> <CoolIcon name="Warning" :size="14" /> <strong>Quan trọng:</strong> đây là SĐT nick Zalo bạn KHÔNG đăng nhập vào CRM.
        Hệ thống sẽ gửi lời mời kết bạn từ "Nick Hệ Thống CRM" tới SĐT này.
        Bạn cần accept trên Zalo cá nhân để hoàn tất.
      </div>

      <div class="ic-info-box"> <CoolIcon name="Lock" :size="14" /> SĐT này CHỈ dùng cho thông báo hệ thống. CRM KHÔNG đọc tin nhắn cá nhân của bạn. </div>
    ...
    <template v-else-if="state.status === 'pending_friend_request'">
      ...
      <div v-if="checkError" class="ic-error"><CoolIcon name="Warning" :size="14" /> {{ checkError }}</div>

      <div class="ic-step-actions">
        <button class="ic-btn-primary" :disabled="checking" @click="onCheckHandshake">
          <span v-if="checking"><CoolIcon name="Timer" :size="14" /> Đang kiểm tra...</span>
          <span v-else>Tôi đã chấp nhận — Kiểm tra ngay</span>
        </button>
        <button class="ic-btn" :disabled="resending" @click="onResendFriendRequest">
          <span v-if="resending"><CoolIcon name="Timer" :size="14" /> Gửi lại...</span>
    ...
    <template v-else-if="state.status === 'pending_user_confirm'">
      ...
      <div v-if="confirmError" class="ic-error"><CoolIcon name="Warning" :size="14" /> {{ confirmError }}</div>

      <div class="ic-step-actions">
        <button class="ic-btn-primary" :disabled="!codeReady || confirming" @click="onConfirmCode">
          <span v-if="confirming"><CoolIcon name="Timer" :size="14" /> Đang xác nhận...</span>
          <span v-else>Xác nhận</span>
        </button>
        <button class="ic-btn" :disabled="resendingCode" @click="onResendVerifyCode">
          <span v-if="resendingCode"><CoolIcon name="Timer" :size="14" /> Gửi lại mã...</span>
          <span v-else>Chưa nhận tin? Gửi lại mã</span>
        </button>
        <button class="ic-btn-text" @click="onReset">Đổi cách thiết lập</button>
      </div>

      <div class="ic-note"> <CoolIcon name="Bulb" :size="14" /> Mã hết hạn sau 30 phút. Nếu sai 5 lần, bạn cần bấm "Gửi lại mã". </div>
    </template>
    ...
    <template v-else-if="state.status === 'uid_not_found' || state.status === 'lookup_failed' || state.status === 'sender_disconnected'">
      <header class="ic-wizard-head">
        <h2><CoolIcon name="Warning" :size="14" /> Setup chưa thành công</h2>
      </header>
...
/* ═══════ INTRO STATE ═══════ */
.ic-intro-head h2 { font-size: 22px; font-weight: 700; color: var(--mc-ink); margin: 0 0 4px; }
.ic-intro-head p { color: var(--mc-text); margin: 0; font-size: 14px; }
...
.ic-why h3 { margin: 0 0 8px; font-size: 16px; font-weight: 700; color: var(--mc-warning); }
.ic-why-sub { color: var(--mc-warning); font-size: 13px; margin: 0 0 12px; }
.ic-why-list { list-style: none; padding: 0; margin: 0 0 12px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px 16px; }
.ic-why-list li { font-size: 13px; color: var(--mc-text); display: flex; align-items: center; gap: 8px; }
...
.ic-why-cta { font-size: 13px; font-weight: 700; color: var(--mc-warning); margin: 0; padding-top: 6px; border-top: 1px dashed #FCD34D; }

.ic-pick-title { font-size: 14px; font-weight: 700; color: var(--mc-ink); margin: 4px 0; }
.ic-cards { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
.ic-card { background: var(--mc-surface); border: 2px solid var(--mc-line); border-radius: 14px; padding: 18px 20px; text-align: left; cursor: pointer; font-family: inherit; transition: border-color 0.15s, transform 0.15s; }
.ic-card:hover { border-color: #aeb7ff; transform: translateY(-1px); }
.ic-card-head { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
.ic-card-icon { font-size: 24px; }
.ic-card h4 { margin: 0; font-size: 15px; font-weight: 700; color: var(--mc-ink); }
.ic-card-desc { font-size: 13px; color: var(--mc-text); line-height: 1.5; margin: 0 0 12px; }
.ic-card-pros { list-style: none; padding: 0; margin: 0 0 14px; display: flex; flex-direction: column; gap: 6px; font-size: 12.5px; color: var(--mc-text); }
.ic-card-cta { color: #aeb7ff; font-weight: 700; font-size: 13px; }

/* ═══════ WIZARD ═══════ */
.ic-wizard-head { display: flex; align-items: center; gap: 12px; padding-bottom: 6px; border-bottom: 1px solid var(--mc-line); }
.ic-wizard-head h2 { font-size: 18px; font-weight: 700; color: var(--mc-ink); margin: 0; }
.ic-back { background: transparent; border: none; color: #aeb7ff; font-weight: 600; font-size: 13px; cursor: pointer; font-family: inherit; padding: 4px 8px; }
.ic-back:hover { text-decoration: underline; }

.ic-nick-list { display: flex; flex-direction: column; gap: 8px; }
.ic-nick-row { display: flex; align-items: center; gap: 14px; padding: 14px 16px; background: var(--mc-surface); border: 2px solid var(--mc-line); border-radius: 10px; cursor: pointer; transition: border-color 0.15s; }
.ic-nick-row:hover:not(.disabled) { border-color: #C7D2FE; }
.ic-nick-row.selected { border-color: #aeb7ff; background: rgba(108,125,232,.14); }
...
.ic-nick-name { font-size: 14px; font-weight: 600; color: var(--mc-ink); display: flex; align-items: center; gap: 8px; }
.ic-nick-warn { background: rgba(248,113,113,.14); color: var(--mc-danger); font-size: 11px; padding: 2px 8px; border-radius: 9999px; font-weight: 700; }
.ic-nick-meta { font-size: 12px; color: var(--mc-muted); }

.ic-tip { font-size: 12px; color: var(--mc-muted); font-style: italic; margin: 4px 0 0; }

.ic-input-label { font-size: 13px; font-weight: 600; color: var(--mc-text); }
.ic-input { padding: 12px 16px; border: 2px solid var(--mc-line); border-radius: 10px; font-size: 15px; font-family: inherit; outline: none; transition: border-color 0.15s; }
.ic-input:focus { border-color: #aeb7ff; }

.ic-warn-box { background: rgba(251,191,36,.14); border: 1px solid #FCD34D; color: var(--mc-warning); padding: 12px 16px; border-radius: 10px; font-size: 13px; line-height: 1.5; }
.ic-info-box { background: rgba(108,125,232,.14); border: 1px solid #C7D2FE; color: #aeb7ff; padding: 12px 16px; border-radius: 10px; font-size: 13px; line-height: 1.5; }

.ic-handshake { background: var(--mc-surface); border: 1px solid var(--mc-line); border-radius: 12px; padding: 18px 22px; }
.ic-handshake-line { font-size: 14px; color: var(--mc-text); margin-bottom: 12px; }
.ic-handshake-steps { padding-left: 20px; font-size: 13.5px; color: var(--mc-text); line-height: 1.8; margin: 0; }

.ic-verify-intro { font-size: 14px; color: var(--mc-text); line-height: 1.6; background: var(--mc-surface-alt); padding: 12px 16px; border-radius: 10px; border: 1px solid var(--mc-line); }
.ic-code-row { display: flex; gap: 10px; justify-content: center; padding: 16px 0; }
.ic-code-input { width: 60px; height: 72px; font-size: 32px; font-weight: 700; text-align: center; border: 2px solid var(--mc-line); border-radius: 12px; font-family: inherit; color: var(--mc-ink); outline: none; transition: border-color 0.15s, box-shadow 0.15s; }
.ic-code-input:focus { border-color: #aeb7ff; box-shadow: 0 0 0 3px rgba(94, 106, 210, 0.15); }

.ic-note { font-size: 12px; color: var(--mc-muted); font-style: italic; }
...
.ic-btn { background: var(--mc-surface); color: var(--mc-text); border: 1px solid var(--mc-line); padding: 11px 18px; border-radius: 10px; font-weight: 600; font-size: 13px; cursor: pointer; font-family: inherit; }
.ic-btn:hover:not(:disabled) { background: var(--mc-surface-alt); }
.ic-btn-text { background: transparent; color: var(--mc-danger); border: none; padding: 11px 8px; font-weight: 600; font-size: 13px; cursor: pointer; font-family: inherit; }
.ic-btn-text:hover { text-decoration: underline; }

.ic-error { background: rgba(248,113,113,.14); color: var(--mc-danger); border: 1px solid #FCA5A5; padding: 10px 14px; border-radius: 8px; font-size: 13px; }
.ic-error-state { background: rgba(248,113,113,.14); color: var(--mc-danger); border: 1px solid #FCA5A5; padding: 16px 20px; border-radius: 12px; font-size: 14px; }
.ic-empty { background: var(--mc-surface-alt); border: 1px dashed var(--mc-line); padding: 24px; border-radius: 12px; text-align: center; color: var(--mc-muted); font-size: 13.5px; }
...
.ic-done-head h2 { margin: 0; font-size: 20px; font-weight: 700; color: var(--mc-ink); }
.ic-done-head p { margin: 2px 0 0; font-size: 13px; color: var(--mc-muted); }

.ic-done-card { background: var(--mc-surface); border: 1px solid var(--mc-line); border-radius: 12px; padding: 16px 20px; }
...
.pill-crm { background: rgba(108,125,232,.14); color: #aeb7ff; }
.pill-phone { background: rgba(251,191,36,.14); color: var(--mc-warning); }
...
.ic-label { color: var(--mc-muted); min-width: 100px; }
.ic-sub { color: #9CA3AF; font-size: 12px; }
code { font-family: ui-monospace, monospace; font-size: 12px; background: var(--mc-surface-alt); padding: 2px 8px; border-radius: 4px; }

.ic-channels h3 { font-size: 14px; font-weight: 700; color: var(--mc-ink); margin: 8px 0; }
.ic-channel-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 12px; }
.ic-channel { font-size: 13px; color: var(--mc-text); padding: 4px 0; }
