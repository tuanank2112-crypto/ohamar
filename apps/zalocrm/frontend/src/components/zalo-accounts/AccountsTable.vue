    ...
    <table class="accounts-table">
      <thead>
        <tr>
          ...
          <th>Hôm nay <span class="th-hint"><CoolIcon name="Archive" :size="14" /><CoolIcon name="Paper_Plane" :size="14" /><CoolIcon name="Settings_Future" :size="14" />🤝<CoolIcon name="Search_Magnifying_Glass" :size="14" /></span></th>
          <th>Hoạt động 7d</th>
      ...
      <tbody>
        ...
        >
              ...
              <div class="info">
                <div class="nm">
                  ...
                  <span v-if="acct.healthAlert" class="badge-alert" title="Uptime < 80% trong 7 ngày">
                    <CoolIcon name="Triangle_Warning" :size="10" />
                  </span>
                </div>
                <div class="uid">
          ...
          <td>
            ...
            >
              ...
              <CoolIcon v-if="acct.canManage" name="Edit_Pencil_01" :size="12" class="owner-edit-icon" />
            </div>
            ...
            <div class="sdk-grid">
              ...
              <template v-for="m in [
                ...
                <div class="sdk-row" :class="sdkBar(acct, m.cat).cls" :title="`${m.lb}: ${sdkBar(acct, m.cat).used}/${sdkBar(acct, m.cat).cap}`">
                  <CoolIcon :name="m.ic" :size="13" class="sk" />
          ...
          <td>
            ...
            <div v-if="acct.metricsToday" class="hn-mt">
              ...
              <div class="hn-mt-row" :title="`Đến: ${acct.metricsToday.msgReceivedFromFriends} bạn / ${acct.metricsToday.msgReceivedFromStrangers} lạ`">
                <div class="label"><span class="em"><CoolIcon name="Archive" :size="14" /></span>Đến</div>
                <div class="friend" :class="{ zero: acct.metricsToday.msgReceivedFromFriends === 0 }"><span class="cell-icon"><CoolIcon name="Users_Group" :size="14" /></span>{{ acct.metricsToday.msgReceivedFromFriends }}</div>
              ...
              <div class="hn-mt-row" :title="`Đi: ${acct.metricsToday.msgSentByUser} sale / ${acct.metricsToday.msgSentByBot} bot`">
                <div class="label"><span class="em"><CoolIcon name="Paper_Plane" :size="14" /></span>Đi</div>
                <div class="user" :class="{ zero: acct.metricsToday.msgSentByUser === 0 }"><span class="cell-icon"><CoolIcon name="User_01" :size="14" /></span>{{ acct.metricsToday.msgSentByUser }}</div>
                <div class="bot" :class="{ zero: acct.metricsToday.msgSentByBot === 0 }"><span class="cell-icon"><CoolIcon name="Settings_Future" :size="14" /></span>{{ acct.metricsToday.msgSentByBot }}</div>
              ...
              <div class="hn-mt-row" :title="`Kết bạn: ${acct.metricsToday.friendReqByUser} thủ công / ${acct.metricsToday.friendReqByBot} auto`">
                ...
                <div class="user" :class="{ zero: acct.metricsToday.friendReqByUser === 0 }"><span class="cell-icon"><CoolIcon name="User_01" :size="14" /></span>{{ acct.metricsToday.friendReqByUser }}</div>
                <div class="bot" :class="{ zero: acct.metricsToday.friendReqByBot === 0 }"><span class="cell-icon"><CoolIcon name="Settings_Future" :size="14" /></span>{{ acct.metricsToday.friendReqByBot }}</div>
              ...
              <div class="hn-mt-row" :title="`Tìm KH: ${acct.metricsToday.phoneSearchByUser} thủ công / ${acct.metricsToday.phoneSearchByBot} auto`">
                <div class="label"><span class="em"><CoolIcon name="Search_Magnifying_Glass" :size="14" /></span>Tìm KH</div>
                <div class="user" :class="{ zero: acct.metricsToday.phoneSearchByUser === 0 }"><span class="cell-icon"><CoolIcon name="User_01" :size="14" /></span>{{ acct.metricsToday.phoneSearchByUser }}</div>
                <div class="bot" :class="{ zero: acct.metricsToday.phoneSearchByBot === 0 }"><span class="cell-icon"><CoolIcon name="Settings_Future" :size="14" /></span>{{ acct.metricsToday.phoneSearchByBot }}</div>
          ...
          <td>
            ...
            <div class="conn-cell">
              <div class="conn-at" :title="acct.lastConnectedAt ? 'Kết nối lúc: ' + fmtDateTime(acct.lastConnectedAt) : 'Chưa từng kết nối'">
                <span class="conn-ic"><CoolIcon name="Link" :size="14" /></span>{{ acct.lastConnectedAt ? fmtDateTime(acct.lastConnectedAt) : '—' }}
              </div>
          ...
          <td class="td-actions" @click.stop>
            ...
            <template v-if="acct.canManage">
              ...
              <button class="icon-btn" title="Mở chi tiết" @click="$emit('open-detail', acct.id)">
                <CoolIcon name="More_Horizontal" :size="16" />
              </button>
            </template>
            <template v-else>
              <button class="icon-btn" title="Xem chi tiết (chỉ đọc — không phải chính chủ)" @click="$emit('open-detail', acct.id)">
                <CoolIcon name="Show" :size="16" />
              </button>
              <span class="readonly-badge" title="Bạn không phải chính chủ nick này — chỉ xem được"><CoolIcon name="Lock" :size="14" /></span>
            </template>
          </td>
        </tr>
        </template>
        <tr v-if="!accounts.length">
          <td colspan="12" class="empty-row">
            <div class="empty-msg">
              <CoolIcon name="Search_Magnifying_Glass" :size="36" />
...
tbody tr:hover { background: var(--mc-surface-alt) }
tbody tr.selected { background: rgba(108,125,232,.14) }
tbody tr.alert { background: rgba(248,113,113,.14) }
tbody tr.alert:hover { background: rgba(248,113,113,.14) }
...
.sdk-cfg-btn {
  margin-left: 8px; border: 1px solid var(--mc-line); background: var(--mc-surface); border-radius: 6px;
  padding: 2px 8px; font-size: 11px; font-weight: 600; color: #2563eb; cursor: pointer;
}
.sdk-cfg-btn:hover { background: rgba(108,125,232,.14); }
.td-sdk { min-width: 160px; }
.sdk-grid { display: flex; flex-direction: column; gap: 3px; }
.sdk-row { display: grid; grid-template-columns: 64px 1fr; align-items: center; gap: 5px; font-size: 11px; color: var(--mc-text); position: relative; }
.sdk-row .sk { color: var(--mc-muted); white-space: nowrap; }
.sdk-row .sv { font-weight: 700; font-variant-numeric: tabular-nums; }
.sdk-row .sv small { font-weight: 500; color: #9ca3af; }
.sdk-row .sbar { grid-column: 1 / -1; height: 3px; background: var(--mc-surface-alt); border-radius: 99px; overflow: hidden; margin-top: 1px; }
.sdk-row .sbar > i { display: block; height: 100%; border-radius: 99px; background: #2563eb; }
.sdk-row.q-warn .sbar > i { background: #f59e0b; }
.sdk-row.q-warn .sv { color: var(--mc-warning); }
...
.uptime.success { color: var(--mc-success) }
.uptime.warning { color: var(--mc-warning) }
.uptime.error { color: var(--mc-danger) }
...
.icon-btn:hover { background: var(--mc-surface-alt); color: var(--mc-ink) }
...
.owner-cell.clickable:hover { background: rgba(108,125,232,.14); }
.owner-cell.clickable:hover .owner-edit-icon { opacity: 1; }
.owner-edit-icon { color: #aeb7ff; opacity: 0; transition: opacity 0.15s; flex-shrink: 0; }
...
.dept-name { font-size: 12px; font-weight: 600; color: var(--mc-ink); }
/* 2026-06-09 — cột Kết nối / Hoạt động */
.conn-cell { display: flex; flex-direction: column; gap: 2px; }
.conn-at { font-size: 11.5px; color: var(--mc-text); font-variant-numeric: tabular-nums; display: inline-flex; align-items: center; gap: 4px; white-space: nowrap; }
...
.dept-role.leader { background: rgba(108,125,232,.14); color: #aeb7ff; }
.dept-role.deputy { background: rgba(251,191,36,.14); color: var(--mc-warning); }
...
.hn-mt-row > div.friend { color: var(--mc-success); background: rgba(16, 185, 129, 0.06); }
.hn-mt-row > div.stranger { color: var(--mc-warning); background: rgba(245, 158, 11, 0.06); }
.hn-mt-row > div.zero { color: var(--mc-muted); font-weight: 400; }
...
.group-count {
  ...
  background: var(--mc-line); padding: 2px 8px; border-radius: 9999px; font-weight: 600;
}
