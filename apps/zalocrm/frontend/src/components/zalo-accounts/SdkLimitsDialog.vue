  ...
  <div :class="embedded ? 'sdk-embed' : 'sdk-overlay'" @click.self="!embedded && $emit('close')">
    <div class="sdk-sheet">
      <div class="sh-head">
        ...
        <button v-if="!embedded" class="x" @click="$emit('close')"><CoolIcon name="Close_MD" :size="16" /></button>
      </div>

      <div class="scope">
        <div class="tab" :class="{ on: tab === 'org' }" @click="tab = 'org'"><CoolIcon name="Globe" :size="14" /> Mặc định hệ thống</div>
        <div class="tab" :class="{ on: tab === 'nick' }" @click="tab = 'nick'">
          <CoolIcon name="Mobile" :size="14" /> Theo nick<span v-if="overrideNickCount > 0" class="badge">{{ overrideNickCount }} nick ghi đè</span>
        </div>
      </div>
      ...
      <div v-else-if="tab === 'org'" class="body">
        <div class="info"><CoolIcon name="Info" :size="15" /> <div><b>Trần mặc định</b> áp cho TẤT CẢ nick. Nick nào cần khác thì sang tab "Theo nick" ghi đè riêng — <b>ưu tiên giá trị của nick, không có thì dùng mặc định này</b>. Mọi chiến dịch (Mục tiêu, Luồng kịch bản, Gửi hàng loạt) đều tuân theo trần này.</div></div>
      ...
      <div v-else class="body">
        <div class="ovr-bar"><CoolIcon name="Settings" :size="14" /> Chỉnh trần riêng cho nick:
        ...
        <div class="info amber"><CoolIcon name="Bulb" :size="15" /> Ô có giá trị = <b>ghi đè</b> (ưu tiên hơn mặc định). Để trống = theo mặc định hệ thống. Xoá ô để bỏ ghi đè.</div>
        ...
        <div v-for="g in GROUPS" :key="g.title" class="grp">
          ...
          <div v-for="cat in g.cats" :key="cat" class="lim">
            ...
            <div>
              <span v-if="nickForm[cat] != null && nickForm[cat] !== ''" class="badge-ovr">GHI ĐÈ <CoolIcon name="Edit_Pencil_01" :size="11" /></span>
      ...
      <div class="foot">
        <span v-if="saveStatus === 'saved'" class="toast ok"><CoolIcon name="Check" :size="13" /> Đã lưu</span>
        <span v-else-if="saveStatus === 'error'" class="toast err"><CoolIcon name="Warning" :size="13" /> {{ saveError }}</span>
        <span v-else class="hint"><CoolIcon name="Save" :size="13" /> Lưu xong áp dụng ngay cho mọi chiến dịch đang chạy.</span>
...
.sdk-sheet { width: 760px; max-width: 100%; max-height: 90vh; background: var(--mc-surface); border-radius: 12px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 12px 40px rgba(0,0,0,.2); }
/* embedded (trong trang Cài đặt): bỏ overlay/dim, panel theo dòng, không clamp chiều cao modal. */
.sdk-embed { width: 100%; }
.sdk-embed .sdk-sheet { width: 100%; max-width: 880px; max-height: none; box-shadow: none; border: 1px solid var(--mc-line); }
.sh-head { display: flex; align-items: flex-start; gap: 12px; padding: 16px 18px; border-bottom: 1px solid var(--mc-line); }
.sh-head .ic { width: 38px; height: 38px; border-radius: 9px; background: rgba(108,125,232,.14); color: #2563eb; display: flex; align-items: center; justify-content: center; font-size: 18px; }
.sh-head h2 { font-size: 16px; font-weight: 700; }
.sh-head .sub { font-size: 12px; color: var(--mc-text); margin-top: 2px; }
.sh-head .x { margin-left: auto; border: none; background: var(--mc-surface-alt); border-radius: 7px; width: 30px; height: 30px; cursor: pointer; font-size: 15px; color: var(--mc-text); }
.scope { display: flex; padding: 0 18px; border-bottom: 1px solid var(--mc-line); background: var(--mc-surface-alt); }
...
.scope .tab .badge { font-size: 10px; background: rgba(108,125,232,.14); color: #2563eb; border-radius: 10px; padding: 1px 7px; margin-left: 5px; font-weight: 700; }
.body { padding: 16px 18px; overflow-y: auto; }
.muted { color: #9ca3af; padding: 30px; text-align: center; }
.info { background: rgba(108,125,232,.14); border: 1px solid #bfdbfe; border-radius: 8px; padding: 10px 12px; font-size: 12px; color: #aeb7ff; margin-bottom: 14px; display: flex; gap: 8px; }
.info.amber { background: rgba(251,191,36,.14); border-color: #fde68a; color: var(--mc-warning); }
...
.inp input { width: 80px; border: 1px solid var(--mc-line); border-radius: 7px; padding: 7px 9px; font-size: 13px; font-weight: 600; text-align: right; }
.inp input:focus { outline: none; border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37,99,235,.12); }
.inp input.ovr { border-color: var(--mc-warning); background: rgba(251,191,36,.14); }
.inp .u { font-size: 11px; color: #9ca3af; }
.ovr-bar { display: flex; align-items: center; gap: 10px; background: rgba(251,191,36,.14); border: 1px solid #fde68a; border-radius: 8px; padding: 9px 12px; margin-bottom: 12px; font-size: 12px; color: var(--mc-warning); }
.ovr-bar select { margin-left: auto; border: 1px solid #fcd34d; border-radius: 6px; padding: 5px 9px; font-size: 12px; background: var(--mc-surface); }
.badge-ovr { font-size: 9.5px; font-weight: 700; background: rgba(251,191,36,.14); color: var(--mc-warning); border-radius: 4px; padding: 1px 6px; }
.badge-def { font-size: 9.5px; font-weight: 700; background: var(--mc-surface-alt); color: #9ca3af; border-radius: 4px; padding: 1px 6px; }
.foot { display: flex; align-items: center; gap: 10px; padding: 13px 18px; border-top: 1px solid var(--mc-line); background: var(--mc-surface-alt); }
.foot .spacer { flex: 1; }
.foot .hint { font-size: 11px; color: #9ca3af; }
.toast { font-size: 12.5px; font-weight: 600; }
.toast.ok { color: #16a34a; } .toast.err { color: #dc2626; }
.btn { border: 1px solid var(--mc-line); background: var(--mc-surface); border-radius: 8px; padding: 8px 15px; font-size: 13px; font-weight: 600; cursor: pointer; color: var(--mc-text); }
