...
<template>
  <v-dialog v-model="show" max-width="980" persistent scrollable>
    <v-card>
      ...
      <v-card-text>
        <!-- FIX U-05 (2026-07-07): wrap in v-form so :rules actually validate -->
        <v-form ref="formRef">
                  ...
                  <tr v-for="f in friendships" :key="f.id">
                    ...
                    <td class="text-caption">
                      <div v-if="f.lastInboundAt || f.lastOutboundAt">
                        ...
                        <span v-if="f.lastOutboundAt" class="text-primary ml-1"> <CoolIcon name="Paper_Plane" :size="14" /> {{ formatRecentDateTime(f.lastOutboundAt) }}
                        </span>
                      </div>
...
const formRef = ref<InstanceType<typeof import('vuetify/components').VForm> | null>(null); // FIX U-05
...
async function onSave() {
  // FIX U-05 (2026-07-07): validate form before saving
  if (formRef.value) {
