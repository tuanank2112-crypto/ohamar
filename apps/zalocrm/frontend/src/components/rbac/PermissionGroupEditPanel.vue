      ...
      <aside class="panel">
        ...
        <div class="panel-body">
          ...
          <section v-if="!isSystem" class="section section-danger">
            ...
            <button class="btn-danger" :disabled="busy || members.length > 0" @click="confirmArchive"> <CoolIcon name="Trash_Empty" :size="14" /> Xóa nhóm quyền </button>
...
async function confirmArchive() {
  if (!props.node) return;
  if (!(await confirm({ title: `Xóa nhóm quyền "${props.node.name}"?`, tone: 'danger', confirmText: 'Xóa' }))) return;
  busy.value = true;
  try {
...
.stat-chip.stat-on { background: rgba(52,211,153,.14); color: var(--mc-success); }
...
.btn-bulk:hover { background: #181d26; color: white; border-color: var(--mc-ink); border-style: solid; }
.btn-bulk:disabled { opacity: 0.4; cursor: not-allowed; }
.bulk-sep { color: #d6d8dc; font-size: 11px; }
.bulk-hint { font-size: 10px; color: var(--mc-muted); font-style: italic; }
...
.th-action:hover { background: var(--mc-surface-alt); }
.th-label {
  display: block;
  font-size: 10px;
  font-weight: 600;
...
.resource-row:hover { background: var(--mc-surface-alt); }
.td-resource {
  padding: 6px 10px;
  background: var(--mc-surface);
  position: sticky;
...
.btn-row-toggle:hover { background: #0a2e0e; color: white; border-color: var(--mc-success); }
...
.hint-warning { background: rgba(248,113,113,.14) !important; border-left-color: var(--mc-danger) !important; color: var(--mc-danger) !important; }
