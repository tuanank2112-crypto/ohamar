...
<template>
  <section class="notes-section">
    ...
    <div class="notes-header">
      <div class="notes-title"> <CoolIcon name="Note_Edit" :size="14" /> Ghi chú <span v-if="rootCount" class="notes-count">#{{ rootCount }}</span>
      </div>
    ...
    <div v-else class="notes-list">
      <article v-for="note in notes" :key="note.id" class="note-card" :data-note-id="note.id">
        ...
        <div v-if="aiNoIntent.has(note.id)" class="ai-suggestion-banner muted">
          <span class="ai-icon"><CoolIcon name="Settings_Future" :size="14" /></span>
    ...
    <Teleport to="body">
      ...
      ><CoolIcon name="Calendar" :size="14" /></div>
    </Teleport>
  </section>
</template>
...
async function onDelete(noteId: string) {
  if (!(await confirm({ title: 'Xoá ghi chú này?', tone: 'danger', confirmText: 'Xoá' }))) return;
...
.ai-suggestion-banner {
  ...
  align-items: center;
  gap: 8px;
  background: linear-gradient(90deg,rgba(251,191,36,.15),rgba(251,146,60,.09));
...
.ai-text { flex: 1; color: var(--mc-warning); }
.ai-text.muted { color: var(--smax-grey-500); font-style: italic; }
.ai-text strong { color: var(--mc-warning); }
.ai-create-btn {
  background: #f57c00;
  color: #fff;
  border: none;
