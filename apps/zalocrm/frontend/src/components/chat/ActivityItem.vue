    ...
    <div class="act-body">
      ...
      <div v-if="autoTagDiff.added.length || autoTagDiff.removed.length" class="autotag-diff">
        <div v-if="autoTagDiff.added.length" class="autotag-row">
          ...
          >
            <CoolIcon :name="autoTagInfo(key).icon" :size="13" class="autotag-emoji" />{{ autoTagInfo(key).label }}
          </span>
        </div>
...
.autotag-arrow.added { color: var(--mc-success); }
.autotag-arrow.removed { color: var(--mc-danger); }
/* Auto-tag chip nhỏ gọn để fit context activity timeline (không cần bằng TagCrmBar) */
.autotag-chip {
  --tag-color: var(--mc-muted);
  display: inline-flex;
  align-items: center;
  gap: 3px;
