  ...
  >
    ...
    <template v-else>
      ...
      <header class="chat-header">
        ...
        >
          <ChevronLeftIcon :size="18" :stroke-width="2.2" />
          <span>Danh sách</span>
        </button>
        ...
        <div class="ch-info">
          ...
          <div class="ch-row-1">
            ...
            <span class="ch-gender-chip" :class="genderChipClass" :title="genderTitle">
              <CoolIcon
                :name="conversation.threadType === 'group' ? 'Users_Group' : contactGender === 'female' ? 'User_03' : contactGender === 'male' ? 'User_01' : 'Circle_Help'"
                :size="15"
                class="gender-svg"
              />
        ...
        <div class="ch-actions">
          <div v-if="conversation.threadType === 'user'" class="rag-chat-controls">
            <select :value="conversation.aiMode || 'OFF'" title="Chế độ AI cho hội thoại" @change="setConversationAiMode(($event.target as HTMLSelectElement).value)">
              <option value="OFF">AI tắt</option><option value="ASSIST">AI gợi ý</option><option value="AUTO">AI tự động</option>
            </select>
            <button v-if="conversation.handoffStatus === 'NONE' || !conversation.handoffStatus" class="rag-takeover" @click="takeOverConversation">Tiếp quản</button>
            <button v-else class="rag-resume" :title="conversation.handoffReason || ''" @click="resumeConversationAi">Bật lại AI</button>
          </div>
          ...
          <v-menu>
            ...
            <v-list density="compact" min-width="220">
              ...
              />
              <!-- 2026-07-13: bỏ 4 mục chưa có backend (Lịch sử / Tìm hội thoại / Tắt
                   thông báo / Báo cáo) — trước chỉ toast "chưa implement", gây hiểu
                   lầm nút hoạt động. Thêm lại khi backend sẵn. -->
            </v-list>
          </v-menu>
        </div>
      </header>
        ...
        <template v-for="item in displayItems" :key="item.key">
            ...
            <div class="msg-album-body">
              <div class="bubble album">
                ...
                <div v-else class="bubble-time">
                  {{ formatMessageTime(item.sentAt) }} · <CoolIcon name="Image_01" :size="14" /> {{ item.messages.length }} ảnh
                </div>
              </div>
            </div>
          </div>
          ...
          >
            <div class="msg-locked-placeholder">
              <CoolIcon name="Lock" :size="13" class="msg-locked-icon" />
      ...
      <div class="input-area">
        ...
        <div class="input-toolbar-top">
          ...
          <!-- Group 2: format -->
          <!-- 2026-07-13: ẩn "Gửi danh thiếp" — chưa có backend (trước chỉ toast
               "chưa implement"). Thêm lại khi API gửi danh thiếp Zalo sẵn sàng. -->
          <button
            class="icon-tool"
        ...
        <div class="input-row">
          ...
          <div ref="editorWrapRef" class="editor-wrap" :class="{ 'editor-locked': !privacyVisibility.canSendInConv(conversation) || isArchivedNick }">
            ...
            >
              <span class="editor-lock-pill"><CoolIcon name="Lock" :size="14" /> Riêng tư — chỉ chính chủ nick gửi được tin</span>
            </div>
            ...
            <div
              ...
              @click.stop
            >
              <span class="editor-lock-pill"><CoolIcon name="Trash_Empty" :size="14" /> Nick đã xóa — không gửi được. Kết nối lại để gửi tin.</span>
            </div>
          </div>
      ...
      <div v-if="albumMenu.open" class="ctx-menu-overlay" @click.self="albumMenu.open = false" @contextmenu.prevent="albumMenu.open = false">
        <div class="ctx-menu album-ctx" :style="{ top: albumMenu.y + 'px', left: albumMenu.x + 'px' }" @click.stop>
          <button class="ctx-item" @click="saveAlbumOne()">
            <span class="ctx-ic"><CoolIcon name="Image_01" :size="14" /></span><span>Lưu ảnh này vào Media</span>
          </button>
...
const emit = defineEmits<{
  ...
  'contact-updated': [payload: { contactId: string; fields: Array<{ field: string; value: unknown }> }];
...
async function setConversationAiMode(mode: string) {
  if (!props.conversation) return;
  try {
    const { data } = await api.patch(`/conversations/${props.conversation.id}/ai-mode`, { mode });
    props.conversation.aiMode = data.aiMode;
    toast.success(`Chế độ AI: ${data.aiMode}`);
  } catch (error: any) { toast.error(error?.response?.data?.error || 'Không đổi được chế độ AI'); }
}

async function takeOverConversation() {
  if (!props.conversation) return;
  try {
    const { data } = await api.post(`/conversations/${props.conversation.id}/handoff`, { reason: 'human_takeover' });
    props.conversation.handoffStatus = data.handoffStatus;
    props.conversation.handoffReason = data.handoffReason;
    toast.success('Sale đã tiếp quản hội thoại');
  } catch (error: any) { toast.error(error?.response?.data?.error || 'Không thể tiếp quản'); }
}

async function resumeConversationAi() {
  if (!props.conversation) return;
  try {
    const { data } = await api.post(`/conversations/${props.conversation.id}/resume-ai`);
    props.conversation.handoffStatus = data.handoffStatus;
    props.conversation.handoffReason = null;
    toast.success('Đã bật lại AI');
  } catch (error: any) { toast.error(error?.response?.data?.error || 'Không thể bật lại AI'); }
}
...
function onAiSuggestionApplied(
  ...
  if (props.conversation?.contact) {
    ...
    emit('contact-updated', { contactId: props.conversation.contact.id, fields: acceptedFields });
  }
}
...
// Click nick chưa KB → mở Zalo profile dialog theo UID đúng nick.
...
async function onPickNick(row: NickCoverageRow) {
  ...
  // Chỉ navigate khi đã KB. Chưa KB → mở profile theo UID đúng nick để sale xử lý tiếp.
  if (row.friendshipStatus !== 'accepted') {
    if (row.zaloUidInNick) {
      userInfoUid.value = row.zaloUidInNick;
      userInfoDialog.value = true;
    }
    toast.push('Nick này chưa kết bạn với KH.');
    return;
  }
...
async function onRemoveFriend() {
  ...
  if (!(await confirm({ title: 'Huỷ kết bạn với KH này?', message: 'Sau đó muốn nhắn lại sẽ phải gửi lời mời kết bạn lại.', tone: 'danger', confirmText: 'Huỷ kết bạn' }))) return;
  actionLoading.value = true;
  try {
...
async function loadTemplates() {
  // /automation/* là EE bundle — Community không mount → 404. Bỏ qua ở CE. (2026-07-13)
...
function onTemplateSelect(payload: { text: string; styles?: Array<{ st: string; start: number; len: number }> }, templateId: string) {
  ...
  slashTriggerPos.value = -1;
  templateQuery.value = '';
  // Track use (non-blocking) — EE only, bỏ qua ở CE (route /automation/* không mount)
  if (isExtension) api.post(`/automation/templates/${templateId}/track-use`).catch(() => {});
}
...
.msg-locked-placeholder {
  ...
  background: var(--mc-surface-alt); border: 1px dashed var(--mc-line); border-radius: 14px;
  padding: 8px 14px; color: #9CA3AF; font-size: 13px; max-width: 70%;
}
.msg-locked-icon { font-size: 16px; color: var(--mc-warning); }
.msg-locked-text { font-style: italic; }
.msg-bubble-wrap.msg-privacy-locked:hover .msg-locked-placeholder { border-color: var(--mc-warning); color: var(--mc-muted); }
...
.empty-state {
  ...
  background:
    radial-gradient(circle at center, rgba(108,125,232,.06), transparent 42%),
    rgba(255,255,255,.012);
}
.empty-state :deep(.v-icon) {
  color: #8f8db0 !important;
  opacity: .92;
}
.empty-state p {
  color: var(--mc-text);
...
/* ════════ Chat header ════════
   Main area keeps customer identity and meta readable; controls stay in the
   right action cluster so the customer name is not squeezed by AI buttons. */
.chat-header {
  position: relative;
...
.chat-back-btn:hover {
  ...
  color: var(--mc-ink);
  border-color: rgba(108,125,232,.42);
}
.chat-back-btn svg { flex-shrink: 0; }
...
/* Two compact info rows below the avatar. */
.ch-row-1 {
  display: flex;
  align-items: center;
  gap: 8px;
...
/* Row 2 keeps remaining meta in one compact scan line. */
.ch-row-chips {
...
/* Compact down common laptop widths without letting controls wrap. */
@media (max-width: 1440px) {
  ...
  .ch-actions { gap: 4px; }
  .rag-chat-controls { gap: 4px; }
  .rag-chat-controls select { width: 84px; }
  .rag-chat-controls button { min-width: 74px; padding: 0 8px; }
...
.ch-row-1 > :not(.ch-name) { flex-shrink: 0; }
.ch-name {
...
.status-accepted { color: var(--mc-success); }
.status-pending { color: var(--mc-warning); }
...
.msg-counts .cnt-in {
  color: var(--mc-success); font-weight: 600;
}
.msg-counts .cnt-out {
...
.virtual-banner {
  ...
  gap: 10px;
  padding: 8px 16px;
  background: linear-gradient(90deg,rgba(251,146,60,.14),rgba(251,191,36,.09));
...
.virtual-banner-sub { font-size: 11px; color: var(--mc-warning); margin-top: 2px; line-height: 1.4; }

/* T11 2026-06-20: banner nick đã xóa — tông XÁM (clone virtual-banner) + chip xám cạnh tên nick */
.archived-banner {
  background: linear-gradient(90deg,var(--mc-panel),var(--mc-surface-alt));
  border-bottom: 1px solid var(--mc-line);
  color: var(--mc-text);
}
.archived-banner .virtual-banner-icon { background: #9ca3af; }
.archived-banner .virtual-banner-sub { color: var(--mc-muted); }
.nick-archived-chip {
  display: inline-flex;
  align-items: center;
  padding: 1px 7px;
...
.btn-action .sub-meta { display: none; }
.friend-hover-group { flex-shrink: 0; }
...
.album-actionbar .ab-btn {
  border: 1px solid var(--mc-line); background: var(--mc-surface); color: var(--mc-text); border-radius: 6px;
  padding: 4px 11px; font-size: 11.5px; cursor: pointer; font-weight: 500;
}
.album-actionbar .ab-btn.primary { background: #181d26; color: #fff; border-color: var(--mc-ink); }
...
.ctx-menu {
  position: fixed; z-index: 101; background: var(--mc-surface); border-radius: 10px;
  box-shadow: 0 10px 28px rgba(15,23,42,.18), 0 2px 6px rgba(15,23,42,.08);
  border: 1px solid var(--mc-line); padding: 6px 0; animation: ctx-pop .12s ease-out;
}
...
.album-ctx .ctx-item {
  ...
  font-size: 13.5px; color: var(--mc-text); background: transparent; border: 0; cursor: pointer;
  text-align: left;
}
.album-ctx .ctx-item:hover { background: var(--mc-surface-alt); }
...
.input-area {
  ...
  --editor-max-h: calc(34dvh - 110px);
}
.input-area :deep(.tag-crm-bar) {
  min-height: 28px;
  padding: 0 0 6px;
  gap: 5px;
  flex-wrap: nowrap;
...
.input-area :deep(.tag-crm-bar::-webkit-scrollbar) {
  height: 0;
}
.input-area :deep(.t2-tag-pill) {
  padding: 2px 9px;
  font-size: 11.5px;
}
.input-area :deep(.tag-add-btn) {
  min-height: 26px;
  padding: 0 10px;
  white-space: nowrap;
}
...
.input-editor :deep(.rich-text-editor) {
  border-radius: 12px;
}
