      ...
      <div class="shm-modal">
        ...
        <footer class="shm-foot">
          ...
          <button
            ...
            @click="onSend"
          >
            <span v-if="sending"><CoolIcon name="Timer" :size="14" /> Đang gửi...</span>
            <span v-else><CoolIcon name="Paper_Plane" :size="14" /> Gửi tới {{ targetName || 'sale' }}</span>
          </button>
        </footer>
      </div>
    </div>
...
.shm-head {
  ...
  background: linear-gradient(180deg,var(--mc-surface),var(--mc-panel));
}
.shm-title {
  font-size: 14px;
  flex: 1;
...
.shm-x:hover { background: var(--mc-surface-alt); }
...
.shm-meta-pill.ok { background: rgba(52,211,153,.14); border-color: #bbf7d0; color: var(--mc-success); }
.shm-meta-pill.warn { background: rgba(251,191,36,.14); border-color: #fde68a; color: var(--mc-warning); }
.shm-meta-pill.from { background: rgba(108,125,232,.14); border-color: #ddd6fe; color: #c4b5fd; }
.shm-meta-pill.arrow { background: transparent; border: none; padding: 3px 2px; color: var(--mc-muted); font-weight: 700; }
...
.shm-regen:hover:not(:disabled) { background: var(--mc-surface-alt); }
...
.shm-btn-ghost:hover { background: var(--mc-surface-alt); }
.shm-btn-primary {
  background: #4f46e5;
  color: #fff;
  border: none;
