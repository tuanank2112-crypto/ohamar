      ...
      <div class="cam-modal" role="dialog" aria-modal="true">
        ...
        <div class="cam-head">
          <span class="cam-ic" :class="tone">
            <CoolIcon :name="tone === 'danger' ? 'Triangle_Warning' : 'Pause'" :size="18" />
          </span>
        ...
        <div v-if="requireReason || collectInput" class="cam-body">
          <label class="cam-label">{{ reasonLabel }} <span v-if="requireReason || inputRequired" class="cam-req">*</span></label>
          <textarea
            ref="reasonEl"
            v-model="reason"
            class="cam-textarea"
          ...
          />
          <p v-if="showError" class="cam-err">Vui lòng nhập {{ reasonLabel.toLowerCase() }}.</p>
        </div>
...
watch(() => props.open, (v) => {
  if (v) {
    reason.value = props.inputInitialValue;
    typed.value = '';
    showError.value = false;
    if (props.requireReason || props.collectInput) void nextTick(() => reasonEl.value?.focus());
...
function onConfirm(): void {
  ...
  if ((props.requireReason || props.inputRequired) && !reason.value.trim()) {
