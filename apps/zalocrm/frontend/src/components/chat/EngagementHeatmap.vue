...
<template>
  <section class="engagement-heatmap" :class="{ 'is-loading': loading }">
    <header class="eh-header">
      <h3 class="eh-title">
        <span class="eh-icon"><CoolIcon name="Chart_Bar_Vertical_01" :size="14" /></span>
        Engagement 4 tuần
      </h3>
    ...
    <template v-else>
      ...
      <div class="eh-breakdown" v-if="breakdown">
        ...
        <div class="bd-item">
          <span class="bd-label"><CoolIcon name="Chat" :size="14" /> Trả lời</span>
          <span class="bd-val">{{ breakdown.replyRate }}%</span>
        </div>
        <div class="bd-item">
          <span class="bd-label"><CoolIcon name="Phone" :size="14" /> Cuộc gọi</span>
          <span class="bd-val">
        ...
        <div class="bd-item">
          <span class="bd-label"><CoolIcon name="Paperclip_Attechment_Tilt" :size="14" /> Ảnh/file/video</span>
...
.eh-pattern.pattern-hot { background: rgba(248,113,113,.14); color: #EF4444; }
.eh-pattern.pattern-champion { background: rgba(251,191,36,.14); color: #F59E0B; }
.eh-pattern.pattern-stable { background: rgba(108,125,232,.14); color: #3B82F6; }
.eh-pattern.pattern-cooling { background: rgba(251,191,36,.14); color: #F97316; }
.eh-pattern.pattern-cold { background: var(--mc-surface-alt); color: #9CA3AF; }
.eh-pattern.pattern-noise { background: var(--mc-surface-alt); color: var(--mc-muted); font-style: italic; }
...
.eh-cell[data-level="0"] { background: var(--mc-surface-alt); }
.eh-cell[data-level="1"] { background: rgba(52,211,153,.14); }
...
.meta-label { color: var(--mc-muted); }
.meta-value { font-weight: 600; }
.meta-value.up { color: #16A34A; }
.meta-value.down { color: #EF4444; }
.meta-value.flat { color: var(--mc-muted); }
.meta-value.score { color: #aeb7ff; }
...
.bd-val { font-weight: 600; color: var(--mc-ink); flex: 0 0 auto; }
.bd-val-sub { font-weight: 500; color: var(--mc-muted); font-size: 10.5px; }
