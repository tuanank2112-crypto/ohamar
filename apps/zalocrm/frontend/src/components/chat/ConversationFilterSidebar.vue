    ...
    <div v-if="collapsed" class="c-content">
      ...
      <div class="c-icon-strip" role="toolbar" aria-label="Bộ lọc sidebar (thu gọn)">
        ...
        <button
          type="button"
          class="c-icon-btn"
          :class="{ active: engagementActiveCount > 0, open: openPopover === 'engagement' }"
          :title="`${TIPS.engagement}${engagementActiveCount > 0 ? ` · ${engagementActiveCount} đang áp` : ''}`"
          aria-label="Lọc theo tương tác"
          @click.stop="togglePopover('engagement')"
        >
          <MessageCircleIcon class="ic" :size="18" :stroke-width="1.9" />
          <span v-if="engagementActiveCount > 0" class="badge">{{ engagementActiveCount }}</span>
        </button>

        <button
          type="button"
          class="c-icon-btn disabled"
          title="Lọc theo thông tin hồ sơ KH (sắp ra mắt)"
          aria-label="Hồ sơ KH sắp ra mắt"
          aria-disabled="true"
          @click.stop.prevent
        >
          <UserCircleIcon class="ic" :size="18" :stroke-width="1.9" />
        </button>

        <button
          type="button"
          class="c-icon-btn disabled"
          title="Lọc theo nguồn KH đến từ đâu (sắp ra mắt)"
          aria-label="Nguồn khách hàng sắp ra mắt"
          aria-disabled="true"
          @click.stop.prevent
        >
          <MegaphoneIcon class="ic" :size="18" :stroke-width="1.9" />
        </button>
      </div>
      ...
      >
        ...
        <div class="po-body">
          ...
          <!-- ENGAGEMENT -->
          <template v-if="openPopover === 'engagement'">
            <div class="po-sub-label">Kiểu tương tác</div>
            <div
              v-for="p in ENGAGEMENT_PATTERNS"
              :key="p.key"
              class="event-row"
              :class="{ checked: filters.state.engagementPatterns.includes(p.key) }"
              :title="p.tip"
              @click="toggleEngagementPattern(p.key)"
            >
              <div class="left">
...
type PopoverKey = 'tag' | 'message' | 'score' | 'time' | 'event' | 'sale' | 'engagement' | 'preset';
...
const POPOVER_META: Record<PopoverKey, { title: string; icon: unknown }> = {
  preset: { title: 'Bộ lọc đã lưu', icon: StarIcon },
  tag: { title: 'Tag', icon: Tag },
  // 2026-06-09 — Nhóm "Tin nhắn" ngay dưới Tag (anh chốt vị trí).
  message: { title: 'Tin nhắn', icon: InboxIcon },
  score: { title: 'Điểm & Trạng thái', icon: Gauge },
  time: { title: 'Thời gian', icon: Clock },
  event: { title: 'Sự kiện sắp tới', icon: CalendarClock },
  sale: { title: 'Sale phụ trách', icon: UserRoundCog },
  engagement: { title: 'Tương tác', icon: MessageCircleIcon },
};
...
const popoverActiveCount = computed(() => {
  switch (openPopover.value) {
    ...
    case 'engagement': return engagementActiveCount.value;
    default: return 0;
  }
});
...
const ICON_DIVIDER_HEIGHT = 13; // 1px line + 4px margin x2 + flex gap after divider
function collapsedPopoverTop(key: PopoverKey): number {
  const step = ICON_BTN_HEIGHT + ICON_GAP;
  let y = ICON_STRIP_PADDING_TOP + step; // folder button sits before popover-capable tools
  const presetVisible = props.filters.presets.value.length > 0;
  if (key === 'preset') return y;
  if (presetVisible) y += step;
  y += ICON_DIVIDER_HEIGHT;
  const afterDividerOrder: PopoverKey[] = [
    'tag',
    ...(!isGroupTab.value ? (['message'] as PopoverKey[]) : []),
    'score',
    'time',
    'event',
    'sale',
  ...
  const index = afterDividerOrder.indexOf(key);
  return y + Math.max(0, index) * step;
}
const popoverStyle = computed(() => {
  if (!openPopover.value) return {};
  return { top: `${collapsedPopoverTop(openPopover.value)}px` };
});
...
async function onCreatePreset() {
  const nameResult = await promptText({ title: 'Lưu bộ lọc', reasonLabel: 'Tên bộ lọc', reasonPlaceholder: 'Ví dụ: Khách cần chăm hôm nay', inputRequired: true, confirmText: 'Tiếp tục' });
  if (!nameResult.ok) return;
  const emojiResult = await promptText({ title: 'Chọn biểu tượng', message: 'Có thể giữ nguyên biểu tượng mặc định.', reasonLabel: 'Emoji', inputInitialValue: '⭐', confirmText: 'Lưu bộ lọc' });
  if (!emojiResult.ok) return;
  await props.filters.createPreset({ name: nameResult.value.trim(), emoji: emojiResult.value.trim() || '⭐' });
}
...
.filter-sidebar.collapsed .ws-collapsed-stack :deep(.lb-group) {
  width: 36px;
  justify-content: center;
  gap: 0;
}
.filter-sidebar.collapsed .ws-collapsed-stack :deep(.lb-badge) {
...
.filter-sidebar.collapsed .ws-collapsed-stack :deep(.lb-text),
.filter-sidebar.collapsed .ws-collapsed-stack :deep(.lb-clock) {
  display: none !important;
}
.filter-sidebar.collapsed .ws-collapsed-stack :deep(.lb-svg) {
...
.c-icon-btn:hover { background: rgba(255,255,255,.045); }
.c-icon-btn.active {
...
.c-icon-btn.open .badge { background: var(--mc-surface); color: #aeb7ff; border-color: #aeb7ff; }
.c-icon-btn .dot-only {
  position: absolute;
  top: 6px;
  right: 8px;
...
.c-footer {
  ...
  align-items: center;
  gap: 4px;
  background: linear-gradient(180deg,transparent 0%,var(--mc-panel) 60%);
...
.c-clear-btn:hover { background: rgba(248,113,113,.14); border-color: #EF4444; }
...
.c-popover::before {
  ...
  border-right: 7px solid var(--mc-surface);
...
.po-close:hover { background: var(--mc-surface-alt); color: var(--mc-ink); }
.po-body {
  padding: 12px 14px;
  overflow-y: auto;
  flex: 1;
...
.po-body::-webkit-scrollbar-thumb { background: var(--mc-line); border-radius: 2px; }
.po-sub-label {
  font-size: 10.5px;
  font-weight: 600;
  color: var(--mc-muted);
...
.po-preset-row:hover { background: var(--mc-surface-alt); }
.po-preset-row.active {
  background: rgba(108,125,232,.16);
  border-color: rgba(108,125,232,.32);
  color: #aeb7ff;
  font-weight: 600;
}
...
.po-create-btn:hover { border-color: var(--mc-primary); background: rgba(108,125,232,.12); }
...
.collapse-btn:hover { background: rgba(108,125,232,.14); color: #c5caff; }
...
.fp-current {
  ...
  min-height: 58px;
  padding: 9px 10px;
  background: linear-gradient(135deg,rgba(108,125,232,.18),rgba(139,92,246,.1));
  border: 1px solid rgba(108,125,232,.34);
  border-radius: var(--mc-radius-lg);
  cursor: pointer;
  font-family: inherit;
  text-align: left;
...
.fp-current:hover {
  border-color: rgba(108,125,232,.58);
  background: linear-gradient(135deg,rgba(108,125,232,.22),rgba(139,92,246,.13));
}
.fp-current:focus-visible { outline: 2px solid var(--mc-primary); outline-offset: 1px; }
...
.fp-sub { font-size: 10.5px; color: var(--mc-muted); display: flex; align-items: center; gap: 3px; }
...
.fp-arrow { font-size: 13px; color: #aeb7ff; font-weight: 700; }
.fp-thumb .single {
  width: 26px;
  height: 26px;
  border-radius: 50%;
...
.fp-back:hover { background: rgba(108,125,232,.12); border-color: var(--mc-primary); }
...
.saved-chip.active { background: rgba(108,125,232,.2); color: #c5caff; }
.saved-chip:hover:not(.active) { border-color: var(--mc-line); }
.saved-chip.add { background: transparent; color: #aeb7ff; border: 1px dashed var(--mc-line); }
...
.sb-body::-webkit-scrollbar-thumb { background: var(--mc-line); border-radius: 3px; }
...
.section-header:hover { background: rgba(255,255,255,.035); }
...
.count-badge.zero { background: var(--mc-surface-alt); color: var(--mc-muted); }
.chevron { color: var(--mc-muted); font-size: 11px; transition: transform 0.15s; }
...
.tag-search .ic { color: var(--mc-muted); font-size: 11px; flex-shrink: 0; }
.tag-search input {
  flex: 1;
  border: none;
  outline: none;
...
.tag-search input::placeholder { color: var(--mc-muted); }
.tag-search-clear {
  background: transparent;
  border: none;
  cursor: pointer;
...
.tag-search-clear:hover { color: #EF4444; background: rgba(248,113,113,.14); }
...
.tag-pill-row.scroll::-webkit-scrollbar-thumb { background: var(--mc-line); border-radius: 2px; }
.tag-pill {
  display: inline-flex;
  align-items: center;
  gap: 4px;
...
.tag-pill:hover { border-color: var(--mc-line); }
.tag-pill.selected { background: #5E6AD2; border-color: #aeb7ff; color: white; }
...
.tag-pill.selected .zalo-dot { background: var(--mc-surface); }
...
.score-input:focus { outline: none; border-color: #aeb7ff; }
.score-track {
  flex: 1;
  height: 4px;
  background: var(--mc-surface-alt);
...
.score-tier-btn.active { background: #5E6AD2; border-color: #aeb7ff; color: white; }
.score-tier-btn:hover:not(.active) { border-color: var(--mc-line); }
...
.stage-chip:hover { border-color: var(--mc-line); }
...
.radio-pill.active { background: #5E6AD2; border-color: #aeb7ff; color: white; }
.radio-pill:hover:not(.active) { border-color: var(--mc-line); }
...
.check-row:hover { background: var(--mc-surface-alt); }
...
.check-row.checked .checkbox { background: #5E6AD2; border-color: #aeb7ff; }
...
.check-row.checked .label { color: #aeb7ff; font-weight: 600; }
.check-row .count {
  font-size: 10.5px;
  color: var(--mc-muted);
  flex-shrink: 0;
...
.event-row:hover { border-color: rgba(108,125,232,.28); background: rgba(255,255,255,.045); }
.event-row.checked { background: rgba(108,125,232,.14); border-color: rgba(108,125,232,.32); }
.event-row .left { display: flex; align-items: center; gap: 8px; }
.event-row .left .icon { font-size: 14px; }
.event-row .left .lbl { color: var(--mc-ink); font-weight: 500; }
.event-row.checked .left .lbl { color: #aeb7ff; font-weight: 600; }
...
.sale-row:hover { background: var(--mc-surface-alt); }
.sale-row .circle {
  width: 13px;
  height: 13px;
  border: 1.5px solid var(--mc-line);
...
.sale-row.checked .circle { border-color: #aeb7ff; }
...
.sale-row.checked .label { color: #aeb7ff; font-weight: 600; }
...
.footer-card {
  background: linear-gradient(135deg,rgba(108,125,232,.14),rgba(139,92,246,.08));
  border: 1px solid rgba(108,125,232,.24);
  border-radius: 8px;
  padding: 9px 10px;
}
.footer-title {
...
.f-chip:hover { border-color: var(--mc-primary); color: #c5caff; }
.f-chip .x { font-size: 9px; color: var(--mc-muted); }
.f-chip:hover .x { color: #aeb7ff; }
...
.clear-all:hover { background: var(--mc-primary); color: white; }
