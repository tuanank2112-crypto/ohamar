...
import { config, envValue } from './config/index.js';
...
import { decryptZaloSession, SAVED_SESSION_WHERE } from './modules/zalo/session-credentials.js';
import { catalogRoutes } from './modules/catalog/catalog-routes.js';
import { knowledgeRoutes } from './modules/knowledge/knowledge-routes.js';
import { obsidianRoutes } from './modules/obsidian/obsidian-routes.js';
import { orderRoutes } from './modules/orders/order-routes.js';
import { ragRoutes } from './modules/ai/rag-routes.js';
...
function configuredCorsOrigins(): string[] {
  const explicit = (envValue('CORS_ORIGINS') || '')
    .split(',')
...
async function bootstrap() {
  ...
  if (config.storageDriver === 'local') {
    if (config.isProduction) {
      app.addHook('preHandler', async (request, reply) => {
        if (!request.url.startsWith('/files/') || request.method === 'OPTIONS') return;
        try {
  ...
  // Phase 1: Ohamar bridge (Zalo I/O via Ohamar gateways — not CRM zca-js)
  const { ohamarBridgeRoutes } = await import('./modules/ohamar-bridge/ohamar-bridge-routes.js');
  await app.register(ohamarBridgeRoutes);
  ...
  try {
    ...
    logger.info(`CRM running on http://${config.host}:${config.port}`);
    ...
    startEngagementCron();
    // Auto-Care 2026-07-08 — follow-up 24h (AI soạn) + chúc mừng sinh nhật + voucher.
    // Mặc định org TẮT; bật qua org config. Gửi qua zaloOps rate-limit, 6-22h VN.
    if (config.nodeEnv !== 'test') {
      const { startAutoCareCron } = await import('./modules/auto-care/auto-care-cron.js');
      startAutoCareCron(io);
      const { startBirthdayCron } = await import('./modules/auto-care/birthday-cron.js');
  ...
  try {
    ...
    const accounts = await prisma.zaloAccount.findMany({
      where: { ...SAVED_SESSION_WHERE, archivedAt: null, zaloUid: { not: null } },
    ...
    for (const account of accounts) {
      const session = decryptZaloSession(account);
      if (session?.imei) {
