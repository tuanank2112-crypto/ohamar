...
export async function backfillAccountHistory(api: any, accountId: string): Promise<BackfillResult> {
  ...
  if (result.friendsSynced > 0 && result.groupsSynced === 0 && dmReceived === 0 && dbCounts[1] === 0) {
    result.warnings = [
      'Zalo did not return old 1-1 chat messages for this account. Friends, aliases and labels were synced; message history will be captured from new incoming/outgoing messages.',
    ];
  }
  return result;
}
