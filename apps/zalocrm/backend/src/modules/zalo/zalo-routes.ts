...
import { decryptZaloSession } from './session-credentials.js';
...
export async function zaloRoutes(app: FastifyInstance): Promise<void> {
  ...
  // Chỉ mở lại sau khi operator đã xử lý challenge/khóa nick và xác minh bằng nick test.
  app.post<{ Params: { id: string } }>('/api/v1/zalo-accounts/:id/safety/resume', async (request, reply) => {
    const { id } = request.params;
    const gate = await requireAccountManagement(request, reply, id);
    if (!gate) return reply;
    const account = await prisma.zaloAccount.findFirst({ where: { id, orgId: request.user!.orgId }, select: { id: true } });
    if (!account) return reply.status(404).send({ error: 'Account not found' });
    return prisma.zaloAccount.update({ where: { id }, data: { safetyStatus: 'active', suspendedAt: null, suspendedReason: null, status: 'disconnected' }, select: { id: true, safetyStatus: true, status: true } });
  });
