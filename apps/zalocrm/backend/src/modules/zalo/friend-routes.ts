...
export async function friendRoutes(app: FastifyInstance) {
  ...
  app.get(`${BASE}-db`, { preHandler: requireGrant('friend', 'access') }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      // Sidebar cần tổng theo mọi nick user được xem, nhưng tuyệt đối không aggregate
      // nick ngoài ACL/phòng ban của user.
      ...
      const [friends, total, countsRaw, perNickRaw] = await Promise.all([
        ...
        // Số bạn bè mỗi nick (cho NickSidebar) — toàn org, không phụ thuộc nick đang
        // xem. 2026-07-13: trước sidebar hiện "—" khi chọn 1 nick cụ thể.
        prisma.friend.groupBy({
          by: ['zaloAccountId'],
          where: { orgId: user.orgId, zaloAccountId: { in: scope.accessibleIds } },
          _count: true,
        }),
      ]);

      const counts = Object.fromEntries(countsRaw.map((g) => [g.relationshipKind, g._count]));
      const countByNick = Object.fromEntries(perNickRaw.map((g) => [g.zaloAccountId, g._count]));
  ...
  app.get('/api/v1/friends-db/all-nicks', { preHandler: requireGrant('friend', 'access') }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const [friends, total, countsRaw, perNickRaw] = await Promise.all([
        ...
        // Số bạn bè mỗi nick (cho NickSidebar) — aggregate toàn bộ, không phụ thuộc
        // trang/lọc hiện tại. 2026-07-13: trước FE derive từ list đã tải → luôn 0.
        prisma.friend.groupBy({
          by: ['zaloAccountId'],
          where: { orgId: user.orgId, zaloAccountId: { in: accessibleIds } },
          _count: true,
