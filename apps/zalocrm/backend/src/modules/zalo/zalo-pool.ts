...
import { decryptZaloSession, encryptedSessionUpdate } from './session-credentials.js';
import { suspendAccountIfRisk } from './account-safety-service.js';
...
class ZaloAccountPool {
  ...
  async reconnect(accountId: string, credentials: ZaloCredentials, proxyUrl?: string | null): Promise<void> {
    ...
    try {
      ...
      const instance = this.instances.get(accountId);
      if (instance) instance.status = 'disconnected';
      const suspended = await suspendAccountIfRisk(accountId, err).catch(() => false);
      await this.updateAccountDB(accountId, suspended ? 'disconnected' : 'qr_pending', null, 'reconnect_failed');
      void this.emitAccountEventToOrg(accountId, suspended ? 'zalo:account-suspended' : 'zalo:reconnect-failed', { accountId, error: String(err), suspended });
    } finally {
  ...
  private saveCredentials(accountId: string, credentials: ZaloCredentials): void {
    ...
    runSystemQuery(() => prisma.zaloAccount
      .update({ where: { id: accountId }, data: encryptedSessionUpdate(credentials) }))
      ...
      if (status === 'connected' && zaloUid !== null) {
        void runSystemQuery(() =>
          prisma.zaloAccount.updateMany({
            ...
            data: { status: 'disconnected', sessionData: Prisma.JsonNull, sessionCiphertext: null },
          }),
        ).then((r) => {
          if (r.count > 0) {
  ...
  private async autoReconnect(accountId: string, expectedEpoch?: number): Promise<void> {
    ...
    try {
      const account = await prisma.zaloAccount.findUnique({
        where: { id: accountId },
        select: { sessionData: true, sessionCiphertext: true, proxyUrl: true, disconnectReason: true },
      });
  ...
  async cleanupStaleGhosts(staleMinutes = 24 * 60): Promise<number> {
    ...
    try {
      return await runSystemQuery(async () => {
        ...
        await prisma.zaloAccount.updateMany({
          where: { id: { in: ids } },
          data: { archivedAt: new Date(), status: 'disconnected', sessionData: Prisma.JsonNull, sessionCiphertext: null },
        });
