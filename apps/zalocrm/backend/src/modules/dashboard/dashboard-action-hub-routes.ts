    ...
    try {
      ...
      const [unrepliedSplit, todayApptSplit, dormantSplit, contactsCount, closedThisMonth] =
        await Promise.all([
          ...
          // vì Contact không có FK zaloAccount; tính thêm KH có hội thoại trên nick của target
          // để contacts từ đồng bộ Zalo chưa gán sale vẫn hiện trên dashboard chủ nick.
          prisma.contact.count({
            where: contactWhereForTarget,
          }),
          // ✅ Chốt tháng (Contact.status='closed_won' hoặc tương đương — TODO map
          // chính xác sau khi anh confirm pipeline status)
          prisma.contact.count({
            where: {
              ...contactWhereForTarget,
      ...
      const [scoreAvg, leadBucketsHi, leadBucketsMid, engBucketsHi, engBucketsMid, prioHigh] = await Promise.all([
        prisma.contact.aggregate({ where: contactWhereForTarget, _avg: { leadScore: true, engagementScore: true } }),
        prisma.contact.count({ where: { ...contactWhereForTarget, leadScore: { gte: 70 } } }),
        prisma.contact.count({ where: { ...contactWhereForTarget, leadScore: { gte: 40, lt: 70 } } }),
        prisma.contact.count({ where: { ...contactWhereForTarget, engagementScore: { gte: 70 } } }),
        prisma.contact.count({ where: { ...contactWhereForTarget, engagementScore: { gte: 40, lt: 70 } } }),
        prisma.contact.count({ where: { ...contactWhereForTarget, priorityScore: { gte: 70 } } }),
      ]);
      ...
      const [statusGroups, msgSentToday, msgRepliedToday, friendsToday, leadsToday] = await Promise.all([
        prisma.contact.groupBy({ by: ['status'], where: contactWhereForTarget, _count: true }),
        ...
        prisma.contact.count({ where: { ...contactWhereForTarget, createdAt: { gte: today, lt: tomorrow } } }),
      ]);
      ...
      const topTagRows = await prisma.$queryRaw<Array<{ tag: string; n: bigint }>>`
        ...
        WHERE c.org_id = ${viewer.orgId}
          AND (
            c.assigned_user_id = ${targetUserId}
            OR EXISTS (
              SELECT 1 FROM conversations cv
              WHERE cv.contact_id = c.id
                AND cv.org_id = ${viewer.orgId}
                AND cv.zalo_account_id = ANY(${ownedNickIds}::text[])
    ...
    try {
      ...
      const perUser = await Promise.all(
        usersInScope.map(async (u) => {
          const ownedUserNicks = await prisma.zaloAccount.findMany({
            where: { orgId: viewer.orgId, ownerUserId: u.id, archivedAt: null },
            select: { id: true },
          });
          const ownedUserNickIds = ownedUserNicks.map((n) => n.id);
          const contactWhereForUser = {
            orgId: viewer.orgId,
            OR: [
              { assignedUserId: u.id },
              {
                conversations: {
                  some: {
                    orgId: viewer.orgId,
                    zaloAccountId: { in: ownedUserNickIds },
                    threadType: 'user',
                    deletedAt: null,
                  },
                },
