...
export async function createPermissionGroup(input: {
  ...
  cloneFromId?: string;
  grants?: GrantsJson;
  actorUserId?: string; // FIX S-04 (2026-07-07): escalation guard
  ...
  // FIX S-04: Escalation guard — grants must be subset of actor's grants
  if (input.actorUserId) {
    await assertNoEscalation(input.actorUserId, grants);
  }
...
export async function updatePermissionGroup(input: {
  ...
  // FIX S-04: Escalation guard — grants must be subset of actor's grants
  if (input.grants !== undefined && input.actorUserId) {
    const sanitized = sanitizeGrants(input.grants);
    await assertNoEscalation(input.actorUserId, sanitized);
  }
...
/**
 * FIX S-04 (2026-07-07): Check that proposed grants are a subset of actor's effective grants.
 * Owner/admin legacy role bypass. Throws if escalation detected.
 */
async function assertNoEscalation(actorUserId: string, proposedGrants: GrantsJson): Promise<void> {
  ...
  for (const [resource, actions] of Object.entries(proposedGrants)) {
    ...
    for (const [action, enabled] of Object.entries(actions as Record<string, boolean>)) {
      if (enabled && !actorGrants[resource]?.[action]) {
        throw new Error(
          `Không thể cấp quyền ${resource}.${action} — bạn không có quyền này.`
        );
      }
    }
  }
