...
async function loadConversation(conversationId: string, orgId: string, take = 40) {
...
export async function generateText(provider: string, apiKey: string, model: string, system: string, prompt: string, maxTokens?: number, baseUrlOverride?: string) {
  ...
  const baseUrl = (baseUrlOverride || providerDef?.baseUrl || '').replace(/\/+$/, '');
  ...
  /* OpenAI, 9router, Qwen, Kimi all use OpenAI-compatible chat/completions API */
  ...
  if (provider === '9router') return generateWithOpenaiCompat(`${baseUrl}/v1/chat/completions`, apiKey, model, system, prompt, maxTokens);
...
export async function generateAiOutput(input: { orgId: string; conversationId: string; type: AiTaskType; messageId?: string }) {
  ...
  const maxTokens = input.type === 'reply_draft' ? 1400 : undefined;
  const raw = await generateText(currentConfig.provider, apiKey, currentConfig.model, system, userPrompt, maxTokens, await getProviderBaseUrl(input.orgId, currentConfig.provider));
...
/* ──────────────────────────────────────────────────────────────────────────
 * Auto-Care follow-up (2026-07-08) — soạn 1 tin chăm sóc cho KH im lặng >24h.
 * Dùng cho auto-care-cron. Tái dùng loadConversation + buildConversationContext.
 * KHÁC generateAiOutput: KHÔNG lưu AiSuggestion (đây là tin GỬI THẬT, không phải
 * gợi ý cho sale). Quota do caller (cron) tự kiểm qua getAiUsage trước khi gọi.
 *
 * catalogContext: mô tả rút gọn sản phẩm active của org (đưa vào để AI gợi giá trị,
 * KHÔNG bịa). Trả text đã trim, hoặc null nếu AI tắt / thiếu key / kết quả rỗng.
 * ────────────────────────────────────────────────────────────────────────── */
export async function generateCareFollowupMessage(input: {
  orgId: string;
  conversationId: string;
  catalogContext?: string;
}): Promise<string | null> {
  const { buildCareFollowupPrompt } = await import('./prompts/care-followup.js');
  const CARE_CONTEXT_MESSAGES = 10; // đọc 10 tin gần nhất kể từ khi KH dừng nhắn
  const [currentConfig, conversation] = await Promise.all([
    getAiConfig(input.orgId),
    loadConversation(input.conversationId, input.orgId, CARE_CONTEXT_MESSAGES),
  ]);
  ...
  const userPrompt = [
    ...
    ...(input.catalogContext?.trim()
      ? ['<product_context>', escapeXmlBoundary(input.catalogContext.trim()), '</product_context>']
      : []),
  ].join('\n');

  const system = buildCareFollowupPrompt(language);
  // 2000: reply chỉ 2-3 câu nhưng reasoning model (vd gemini-*-pro) tiêu token cho
  // reasoning trước khi xuất → 600 chạm trần MAX_TOKENS. Cho headroom.
  const raw = await generateText(
    currentConfig.provider, apiKey, currentConfig.model, system, userPrompt, 2000,
    await getProviderBaseUrl(input.orgId, currentConfig.provider),
  );
  const text = raw?.trim();
...
export async function parseAppointmentFromText(input: { orgId: string; text: string; now?: Date }): Promise<ParsedAppointment & { source?: 'ai' | 'fallback' } | null> {
  ...
  const system = [
    ...
    '- HOẶC từ khoá hành động hẹn: "gọi lại", "nhắn lại", "gặp", "ghé", "đến", "đặt lịch", "tái khám", "qua văn phòng", "tới chỗ"',
    '- HOẶC từ khoá địa điểm: "tại [địa điểm]", "ở [địa điểm]", "[tên cơ sở/đường/quận]", "VP", "phòng khám", "showroom", "cơ sở [name]"',
    ...
    'type rules: "gọi"/"call"→call, "nhắn"→message, "gặp"/"ghé"/"đến"/"đặt lịch"/"tái khám"→meeting, fallback→follow_up.',
 ...
 * nội dung sản phẩm/dịch vụ) → AI return {text, styles[]} format Zalo (bold/italic/
...
VÍ DỤ INPUT:
"- Sản phẩm dưỡng da A có tại cơ sở Q.7\\n- Giá tham khảo 500.000đ (giảm 10%)\\n- Hotline: 0901-123-456"

VÍ DỤ OUTPUT:
{"ranges":[
  {"phrase":"Sản phẩm dưỡng da A","styles":["b","c_db342e"]},
  {"phrase":"Q.7","styles":["b","c_f27806"]},
  {"phrase":"500.000đ","styles":["b","c_db342e"]},
