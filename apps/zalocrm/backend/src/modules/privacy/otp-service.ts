...
const OTP_RESEND_COOLDOWN_MS = 60 * 1000;      // FIX S-12: 60s giữa 2 lần request
...
// FIX S-12: HMAC với encryptionKey
function hashOtp(code: string): string {
  return createHmac('sha256', config.encryptionKey).update(code).digest('hex');
}
...
> *Nếu không phải anh/chị yêu cầu, vui lòng bỏ qua tin này.*`;
  ...
  const limits = await zaloRateLimiter.reserveSend(args.senderId, 'message');
  if (!limits.allowed) {
