...
export async function rotateRefreshToken(
  ...
  // 5. Đường bình thường: atomically claim this token for rotation before issuing
  // the replacement. This prevents two concurrent refreshes from both seeing
  // usedAt=null and minting independent successors.
  const usedAt = new Date();
  const claimed = await runSystemQuery(() =>
    prisma.refreshToken.updateMany({
      where: { id: rec.id, usedAt: null, revokedAt: null },
      data: { usedAt },
    }),
  );
  if (claimed.count !== 1) {
    const latest = await runSystemQuery(() =>
      prisma.refreshToken.findUnique({
        where: { id: rec.id },
    ...
    if (latest?.usedAt) {
      const sinceUsed = Date.now() - latest.usedAt.getTime();
      if (sinceUsed <= config.refreshGraceMs && latest.expiresAt.getTime() > Date.now()) {
        const graceIssued = await issueRefreshToken(rec.userId, meta, rec.familyId, latest.expiresAt);
        return { ...graceIssued, userId: rec.userId };
      }
    }
    return reuse('concurrent_rotation_lost');
  }
