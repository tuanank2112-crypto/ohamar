...
export async function authMiddleware(
  ...
  if (claim?.id) {
    // FIX S-08: enforce revocation on every authenticated request, including
    // short-lived access tokens. Logout/password reset/admin lock bump or check
    // jwtTokenVersion/isActive so leaked tokens cannot keep data access for 15m.
    const userModel = (prisma as typeof prisma & { user?: typeof prisma.user }).user;
    const skipDbRecheckForLegacyUnitMock = process.env.NODE_ENV === 'test' && !userModel?.findUnique;
    if (!skipDbRecheckForLegacyUnitMock) {
      ...
      if (typeof claim.tv === 'number' && dbUser.jwtTokenVersion !== claim.tv) {
