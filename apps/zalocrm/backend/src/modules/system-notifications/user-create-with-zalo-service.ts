...
/**
 * Sinh password tạm đủ mạnh cho policy onboarding: có hoa, thường, số; user vẫn
 * bị force-change-first-login nên plaintext chỉ tồn tại trong luồng cấp phát ban đầu.
 */
export function generateTempPassword(): string {
  ...
  for (let i = 3; i < bytes.length; i++) chars.push(all[bytes[i] % all.length]);
...
async function sendWelcomeAndFallback(p: SendLoginParams): Promise<CreateUserResult['zalo']> {
  ...
  const limits = await zaloRateLimiter.reserveSend(senderId, 'message');
  if (!limits.allowed) {
    return {
    ...
    try {
      ...
      if (adminPhoneNorm) {
        ...
        if (adminUid) {
          ...
          const fallbackLimit = await zaloRateLimiter.reserveSend(senderId, 'message');
          if (!fallbackLimit.allowed) throw new Error(fallbackLimit.reason || 'Rate limit');
          const api = zaloPool.getApi(senderId);
          if (api) {
...
export async function createUserAndSendLogin(input: CreateUserInput): Promise<CreateUserResult> {
  ...
  // Re-check phone uniqueness within org. Same phone can exist in another tenant.
  const dup = await prisma.user.findFirst({
    where: { orgId: input.orgId, phone: normalizedPhone },
  ...
  if (trimmedEmail) {
    const dupEmail = await prisma.user.findFirst({ where: { orgId: input.orgId, email: trimmedEmail } });
    if (dupEmail) {
