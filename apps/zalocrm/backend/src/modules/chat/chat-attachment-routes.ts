    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (imageIndexes.length > 0) {
          const imageLimit = await zaloRateLimiter.reserveSend(conversation.zaloAccountId);
          if (!imageLimit.allowed) return reply.status(429).send({ error: imageLimit.reason });
        ...
        for (const i of videoIndexes) {
          const videoLimit = await zaloRateLimiter.reserveSend(conversation.zaloAccountId);
          if (!videoLimit.allowed) return reply.status(429).send({ error: videoLimit.reason });
