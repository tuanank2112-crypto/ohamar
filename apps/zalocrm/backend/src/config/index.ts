...
export const config = {
  ...
  allowZaloCredentialExport: (envValue('ALLOW_ZALO_CREDENTIAL_EXPORT') || 'false').toLowerCase() === 'true',
  ...
  vertexProjectId: envValue('VERTEX_PROJECT_ID') || envValue('GOOGLE_CLOUD_PROJECT') || '',
  vertexLocation: envValue('VERTEX_LOCATION') || 'asia-southeast1',
  vertexEmbeddingModel: envValue('VERTEX_EMBEDDING_MODEL') || 'gemini-embedding-001',
  googleCloudAccessToken: envValue('GOOGLE_CLOUD_ACCESS_TOKEN') || '',
  ragAllowLocalEmbeddings: (envValue('RAG_ALLOW_LOCAL_EMBEDDINGS') || 'false').toLowerCase() === 'true',
  obsidianSyncEnabled: (envValue('OBSIDIAN_SYNC_ENABLED') || 'false').toLowerCase() === 'true',
  obsidianVaultPath: envValue('OBSIDIAN_VAULT_PATH') || '',
  ...
  nineRouterBaseUrl: envValue('NINE_ROUTER_BASE_URL') || '',
  nineRouterAuthToken: envValue('NINE_ROUTER_AUTH_TOKEN') || envValue('NINE_ROUTER_API_KEY') || '',
  ...
  cspMode: (() => {
    const v = (envValue('CSP_MODE') || (isProd ? 'enforce' : 'report-only')).toLowerCase();
