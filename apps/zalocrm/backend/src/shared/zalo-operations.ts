...
import { decryptZaloSession } from '../modules/zalo/session-credentials.js';
...
async function attemptReconnect(accountId: string): Promise<void> {
  ...
  const attempt = (async () => {
    const account = await prisma.zaloAccount.findUnique({
      where: { id: accountId },
      select: { sessionData: true, sessionCiphertext: true },
    });
...
async function exec<T>(opts: ExecOptions, fn: (api: any) => Promise<T>): Promise<T> {
  ...
  // 2. Atomic rate-limit reservation. Counting before the SDK call is intentional:
  // concurrent sends must not all pass the check before any counter is incremented.
  const limit = await zaloRateLimiter.reserveSend(accountId, category);
  if (!limit.allowed) {
      ...
      if (attempt === 0 && isSessionExpiredError(err)) {
        ...
        try {
          ...
          if (freshInstance?.api && freshInstance.status === 'connected') {
            ...
            return await fn(freshInstance.api);
          }
        } catch (reconnectErr) {
